...
namespace ORB_SLAM2 {
static mutex mMutexOptimizer; //优化锁
bool comp_sort(const KeyFrame *a, const KeyFrame *b);
...
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool *pbStopFlag,
  ...
  list<KeyFrame *>
      lLocalKeyFrames; // local KFs which share map points with current frame.
  ...
  // Local MapPoints seen in Local KeyFrames. some points might not be seen by
  // currentKF
  ...
  for (list<MapPoint *>::iterator lit = lLocalMapPoints.begin(),
    ...
    for (map<KeyFrame *, size_t>::iterator mit = observations.begin(),
      ...
      if (pKFi->mnBALocalForKF != pKF->mnId &&
          pKFi->mnBAFixedForKF !=
              pKF->mnId) // not been added to lLocalKeyFrames, and
                         // lFixedCameras!
      {
  ...
  if (showlog) {
    cout << "Optimizer localKFs: " << lLocalKeyFrames.size() << " "
         << "localMPs: " << lLocalMapPoints.size()
         << "lfixeds: " << lFixedCameras.size() << endl;
  }
  ...
  vector<g2o::EdgeStereoSE3ProjectXYZ *>
      vpEdgesStereo; // left + right camera + map point
  ...
  for (list<MapPoint *>::iterator lit = lLocalMapPoints.begin(),
    ...
    if (pMP->Observations() == 1)
      // by me, skip observation one point, which is just initialized by my
      // depth! won't affect previous mono.
      continue;
...
void Optimizer::LocalBACameraPointObjects(KeyFrame *pKF, bool *pbStopFlag,
                                          Map *pMap, bool fixCamera,
                                          bool fixPoint) {
  unique_lock<mutex> lock(mMutexOptimizer);
  bool showlog = false;
  // Local KeyFrames to optimize: First Breath Search from Current Keyframe
  ...
  for (int i = 0, iend = vNeighKFs.size(); i < iend; i++) {
    ...
    //            pKFi->mnBALocalForKF = pKF->mnId;
    // lin755<
    if (!pKFi->isBad() && pKFi->mnId < pKF->mnId) {
      if (pKFi->mnId > pKF->mnId) {
        cout << "[ERROR] pKFi->mnId > pKF->mnId " << pKFi->mnId << " > "
             << pKF->mnId << endl;
      }
      pKFi->mnBALocalForKF = pKF->mnId;
      lLocalKeyFrames.push_back(pKFi);
  ...
  for (vector<KeyFrame *>::iterator lit = lLocalKeyFrames.begin(),
    ...
    for (vector<MapObject *>::iterator vit = vpMOs.begin(), vend = vpMOs.end();
         vit != vend; vit++) {
      MapObject *pMO = *vit;
      if (pMO)
        if (!pMO->isBad())
  ...
  for (vector<MapPoint *>::iterator lit = lLocalMapPoints.begin(),
    ...
    for (map<KeyFrame *, size_t>::iterator mit = observations.begin(),
      ...
      if (pKFi->mnBALocalForKF != pKF->mnId &&
          pKFi->mnBAFixedForKF != pKF->mnId &&
          pKFi->mnId < pKF->mnId) // not been added to lLocalKeyFrames, and
                                  // lFixedCameras! // lin755
      {
        pKFi->mnBAFixedForKF = pKF->mnId;
        if (!pKFi->isBad())
          lFixedCameras.push_back(pKFi);
  ...
  // More Fixed Keyframes. Keyframes that see Local MapObjects but that are not
  // Local Keyframes  改变的地方
...
#define ObjectFixScale // use when scale is provided and fixed. such as KITTI
  ...
  g2o::BlockSolverX::LinearSolverType
      *linearSolver; // BlockSolverX instead of BlockSolver63
  linearSolver =
  ...
  // addvertex error
  vector<double> a;
  for (vector<KeyFrame *>::iterator lit = lFixedCameras.begin(),
                                    lend = lFixedCameras.end();
       lit != lend; lit++) {
    KeyFrame *pKFi = *lit;
    a.push_back(pKFi->mnId);
    g2o::VertexSE3Expmap *vSE3 = new g2o::VertexSE3Expmap();
    vSE3->setEstimate(Converter::toSE3Quat(pKFi->GetPose()));
    vSE3->setId(pKFi->mnId);
    vSE3->setFixed(true);
  ...
  for (vector<MapObject *>::iterator lit = lLocalMapObjects.begin(),
    ...
    if (scene_unique_id == kitti)
      ...
      ROS_ERROR_STREAM(
          "Please see cuboid scale!!!, otherwise use VertexCuboid()");
    ...
    if (scene_unique_id == kitti) {
      if (!build_worldframe_on_ground) {
        ...
        cube_pose.setTranslation(Vector3d(cube_pose.translation()(0),
                                          cube_pose.translation()(1),
                                          cam_height - 1.0));
      }
      if (vObject->fixedscale(0) >
          0) // even later didn't actually optimize, change the scale. just for
             // visualization.
  ...
  for (vector<MapPoint *>::iterator lit = lLocalMapPoints.begin(),
    ...
    for (map<KeyFrame *, size_t>::const_iterator mit = observations.begin(),
      ...
      if (!pKFi->isBad()) {
        ...
        // BUG
        if (pKFi->mvuRight[mit->second] < 0) {
          if (optimizer.vertex(pKFi->mnId) == 0 || optimizer.vertex(id) == 0) {
            cout << optimizer.vertex(id) << " " << optimizer.vertex(pKFi->mnId)
                 << " " << pKFi->mnId << " " << pKFi->isBad() << endl;
            continue;
          }
  ...
  if (1) {
    // 此为一处可改进点
    // 当物体生成时获取物体点的关联，而不必每一帧都做
    // get object point association.can we do this when validate cuboids?? then
    ...
    for (size_t i = 0; i < lLocalMapObjects.size(); i++) {
      ...
      point_object_threshold =
          std::max(int(largest_point_obs_num * 0.4),
      ...
      for (size_t j = 0; j < UniquePoints.size(); j++)
        if (UniquePoints[j])
          if (!UniquePoints[j]->isBad())
            if (UniquePoints[j]->MapObjObservations[pMO] >
                point_object_threshold) {
    ...
    for (size_t i = 0; i < lLocalMapObjects.size(); i++) {
      ...
      for (size_t j = 0; j < all_object_ba_points[i].size(); j++)
        if ((mean_point - all_object_ba_points[i][j]).norm() <
            coarse_threshold) {
      ...
      std::vector<Eigen::Vector3d>
          good_points; // for car, if points are 4 meters away from center,
                       // usually outlier.
      for (size_t j = 0; j < all_object_ba_points[i].size(); j++) {
        if ((mean_point_2 - all_object_ba_points[i][j]).norm() <
            fine_threshold) {
          mean_point_final += all_object_ba_points[i][j];
          good_points.push_back(all_object_ba_points[i][j]);
          pMObj->used_points_in_BA_filtered.push_back(
              pMObj->used_points_in_BA[j]);
        }
      ...
      if ((all_object_ba_points[i].size() > 5) &&
          1) // whether want to initialize object position to be center of
             // points
      {
        g2o_object_vertex *vObject = static_cast<g2o_object_vertex *>(
            optimizer.vertex(pMObj->mnId + maxKFid + 1));
    ...
    for (size_t i = 0; i < lLocalMapObjects.size();
         i++) // no need to optimize all objects...., use local KF's map
              // objects?
    {
      MapObject *pMO = lLocalMapObjects[i];
...
#else
        g2o::EdgePointCuboidOnlyObject *e =
            new g2o::EdgePointCuboidOnlyObject();
#endif
        for (size_t j = 0; j < all_object_ba_points[i].size(); j++)
      ...
      if (0) // each object is connect to one point. optimize both point and
             // object
      {
      ...
      // "<<pMO->used_points_in_BA_filtered.size()<<"
      // "<<all_object_ba_points[i].size());
    }
  }
  ...
  if (1) {
    ...
    inv_sigma =
        inv_sigma *
        camera_object_BA_weight; // point sigma<1, object error is usually
    ...
    Eigen::Matrix4d camera_object_sigma =
        inv_sigma.cwiseProduct(inv_sigma).asDiagonal();
    const float thHuberObject =
        sqrt(900); // object reprojection error is usually large
    int total_left = 0;
    int total_right = 0;
    int total_middle = 0;
      ...
      for (vector<MapObject *>::iterator lit = lLocalMapObjects.begin(),
        ...
        for (unordered_map<KeyFrame *, size_t>::const_iterator
          ...
          if (!pKFi->isBad()) {
            ...
            e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex *>(
                                optimizer.vertex(pMObject->mnId + maxKFid +
                                                 1))); // object
        ...
        if (obj_obs_num == 1) // if an object is only connected to one camera,
                              // don't optimize it! no need
        {
          obj_edge->setLevel(1);
        }
      }
  ...
  for (vector<KeyFrame *>::iterator lit = lLocalKeyFrames.begin(),
    ...
    if (false) { // once
      once = false;
      cv::Mat Tcw;
      Converter::toCvMat(SE3quat).copyTo(Tcw);
      cv::Mat Rcw = Tcw.rowRange(0, 3).colRange(0, 3);
      cv::Mat tcw = Tcw.rowRange(0, 3).col(3);
      cv::Mat Rwc = Rcw.t();
      cv::Mat Ow = -Rwc * tcw; // 世界坐标
      cv::Mat Twc_Old = pKFrame->GetPoseInverse();
      cv::Mat twc_Old = Twc_Old.rowRange(0, 3).col(3);
      cv::Mat t = Ow - twc_Old;
      P_t = t;

      vector<KeyFrame *> vpKFs = pMap->GetAllKeyFrames();
      sort(vpKFs.begin(), vpKFs.end(), comp_sort);
      for (std::vector<KeyFrame *>::iterator iter = vpKFs.begin();
           iter != vpKFs.end(); iter++) {
        if ((*iter)->mnId > pKFrame->mnId &&
            vpKFs.back()->mnId - (*iter)->mnId >
                10) { // && vpKFs.back()->mnId - (*iter)->mnId > 20 &&
                      // (*iter)->mnId - pKF->mnId < 20
          //                        cout<<"[WARN] 尝试修正位姿id:
          //                        "<<(*iter)->mnId<<endl;
          //修正点
          cv::Mat Twc_pose_old = (*iter)->GetPoseInverse();
          cv::Mat twc_pose_old = Twc_pose_old.rowRange(0, 3).col(3);
          cv::Mat twc_pose_new = twc_pose_old + t;
          cv::Mat Twc_pose_new = Twc_pose_old;
          twc_pose_new.copyTo(Twc_pose_new.rowRange(0, 3).col(3));
          // Twc转Tcw
          cv::Mat Rwc = Twc_pose_new.rowRange(0, 3).colRange(0, 3);
          cv::Mat twc = Twc_pose_new.rowRange(0, 3).col(3);
          cv::Mat Rcw = Rwc.t();
          cv::Mat tcw = -Rcw * twc; // 世界坐标
          //赋值
          cv::Mat Tcw_New = cv::Mat::eye(4, 4, Tcw.type());
          Rcw.copyTo(Tcw_New.rowRange(0, 3).colRange(0, 3));
          tcw.copyTo(Tcw_New.rowRange(0, 3).col(3));
          (*iter)->SetPose(Tcw_New);
          //插入关键点
          set<MapPoint *> points = (*iter)->GetMapPoints();
  ...
  //        iter_p=allpoints.begin();iter_p!=allpoints.end();iter_p++){
  //            (*iter_p)->SetWorldPos((*iter_p)->GetWorldPos()+P_t);
  //        }
  ...
  for (vector<MapObject *>::iterator lit = lLocalMapObjects.begin(),
    ...
    g2o_object_vertex *vObject = static_cast<g2o_object_vertex *>(
        optimizer.vertex(pMObject->mnId + maxKFid + 1));
...
// similar to localBA, add objects
void Optimizer::LocalBACameraPointObjectsDynamic(KeyFrame *pKF,
  ...
  cout << "delete old single map points   " << deleteoldsinglepts << "   "
       << deleteoldsinglepts2 << endl;
  ...
  for (vector<MapObject *>::iterator lit = lLocalMapObjects.begin(),
    ...
    for (unordered_map<KeyFrame *, size_t>::iterator mit = observations.begin(),
      ...
      // NOTE if frame too old, skip it. dynamic objects can be observed by many
      // frames!!!
      if ((pKFi->mTimeStamp - pKF->mTimeStamp) > 8.0 &&
          pKFi->mnBALocalForKF != pKF->mnId &&
          pKFi->mnBAFixedForKF !=
              pKF->mnId) // not been added to lLocalKeyFrames, and
                         // lFixedCameras!
...
#define ObjectFixScale
...
#ifdef ObjectFixScale
  g2o::BlockSolverX::LinearSolverType
      *linearSolver; // HACK BlockSolverX  BlockSolver_6_3
  linearSolver = new g2o::LinearSolverDense<
      g2o::BlockSolverX::PoseMatrixType>(); // LinearSolverEigen
                                            // LinearSolverDense
  g2o::BlockSolverX *solver_ptr = new g2o::BlockSolverX(linearSolver);
#else
  g2o::BlockSolverX::LinearSolverType
    ...
    for (unordered_map<KeyFrame *, size_t>::iterator mit = observations.begin(),
      ...
      if (scene_unique_id == kitti)
        vObject->fixedscale = Eigen::Vector3d(
            1.9420, 0.8143,
            0.7631); // for kitti object, scale may don't need to set...
      else
        ROS_ERROR_STREAM(
            "Please see cuboid scale!!!, otherwise use VertexCuboid()");
      ...
      if (scene_unique_id == kitti) {
        if (!build_worldframe_on_ground) {
          ...
          float cam_height = pKFi->GetCameraCenter().at<float>(
              2); // in init frame, y downward for KITTI
        }
        if (vObject->fixedscale(0) >
            0) // even later didn't actually optimize, change the scale. just
               // for visualization.
          cube_pose.setScale(vObject->fixedscale);
      }
#endif
  ...
  for (vector<MapPoint *>::iterator lit = lLocalMapPoints.begin(),
    ...
    if (pMP->Observations() ==
        1) // HACK by me, skip observation one point, which is initialized by my
           // depth! won't affect previous mono.
      continue;

    if (pMP->is_dynamic) // for dynamic point, treat separatedly, use
                         // object-point-camera edge.
      continue;
    ...
    if (fixPoint)
      ...
      vPoint->setMarginalized(
          true); // must be true, otherwise run error. Not sure why. from g2o
                 // paper, marginalize is just one way to solve...
    optimizer.addVertex(vPoint);
  ...
  vpEdgesCameraPointObject.reserve(
      nExpectedSize); // a camera + map point + object
  ...
  if (ba_dyna_pt_obj_cam) {
    for (vector<MapPoint *>::iterator lit = lLocalMapPoints.begin(),
      ...
      if (pMP->Observations() <
          4) // 3 should have more frame observations... otherwise object depth
             // inited map point is not accurate.
        continue;
      ...
      vPoint->setEstimate(Converter::toVector3d(
          pMP->PosToObj)); // dynamic point use point-object pose
      ...
      for (map<KeyFrame *, size_t>::const_iterator mit = observations.begin(),
        ...
        if (!pKFi->isBad()) {
          ...
          // point-object-camera
          g2o::EdgeDynamicPointCuboidCamera *e =
              new g2o::EdgeDynamicPointCuboidCamera();

          e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(
                              optimizer.vertex(pKFi->mnId))); // camera
          e->setVertex(
              1, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(
  ...
  cout << "BA vpEdgesCameraPointObject size   "
       << vpEdgesCameraPointObject.size() << endl;
  ...
  if (ba_dyna_obj_velo) {
    ...
    inv_sigma(2) =
        inv_sigma(2) *
        5.0; // 0.1 angle error is usually much smaller compared to position.
    inv_sigma = inv_sigma * object_velocity_BA_weight;
    Eigen::Matrix3d object_velocity_sigma =
        inv_sigma.cwiseProduct(inv_sigma).asDiagonal();
    ...
    for (vector<MapObject *>::iterator lit = lLocalMapObjects.begin(),
      ...
      if (pMObject->bundle_vertex_ids.size() <
          4) // 5 if only with few frames, don't optimize velocity. not
             // accurate. meaningful for at least 3 objs
        continue;

      int id = pMObject->mnId + maxIdTillPoint + 1;
      g2o::VelocityPlanarVelocity *vVelocity =
          new g2o::VelocityPlanarVelocity();
      ...
      for (size_t i = 0; i < objkfs.size(); i++) {
        ...
        if (!obskf->isBad() && pMObject->bundle_vertex_ids.count(obskf)) {
          ...
          } else // add velocity edge
          {
            ...
            e->setVertex(
                2, dynamic_cast<g2o::OptimizableGraph::Vertex *>(vVelocity));
            e->delta_t = obskf->mTimeStamp - prevgoodobsframe->mTimeStamp;
            // 		  cout<<"add obj id for frames:   "<<pMObject->mnId<<"
            // "<<prevgoodobsframe->mnId<<"   "<<obskf->mnId<<"    delta
            // "<<e->delta_t <<endl; 		  cout<<"obskf->mTimeStamp
            // "<<obskf->mTimeStamp<<"   "<<prevgoodobsframe->mTimeStamp<<endl;
      ...
      if (pMObject->velocityPlanar(0) == 0 &&
          pMObject->velocityPlanar(1) == 0 && firstgoodobsframe &&
          lastgoodobsframe) {
        // compute linear velocity, angular velocity
        double time_range =
            lastgoodobsframe->mTimeStamp - firstgoodobsframe->mTimeStamp;
        g2o::cuboid init_pose =
            pMObject->allDynamicPoses[firstgoodobsframe].first;
        g2o::cuboid last_pose =
            pMObject->allDynamicPoses[lastgoodobsframe].first;
        double linear_esti =
            (last_pose.pose.translation() - init_pose.pose.translation())
                .norm() /
            time_range;
  ...
  if (ba_dyna_obj_cam) // for kitti, this works better as scale is given
  {
    ...
    Eigen::Matrix4d camera_object_sigma =
        inv_sigma.cwiseProduct(inv_sigma).asDiagonal();
    const float thHuberObject =
        sqrt(900); // 900 object reprojection error is usually large
    int total_left = 0;
    int total_right = 0;
    int total_middle = 0;
    vector<g2o_camera_obj_2d_edge *> vpEdgesCameraObjectLeft;
        ...
        for (unordered_map<KeyFrame *, size_t>::const_iterator
          ...
          if (!pKFi->isBad()) {
            ...
            e->setVertex(
                1,
                dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(
                    pMObject->bundle_vertex_ids[pKFi]))); // object
  ...
  for (vector<MapObject *>::iterator lit = lLocalMapObjects.begin(),
    ...
    pMObject->pose_Twc_latestKF =
        pMObject->allDynamicPoses[latest_observed_goodframe].first;
    ...
    g2o::VelocityPlanarVelocity *vVelocity =
        dynamic_cast<g2o::VelocityPlanarVelocity *>(
            optimizer.vertex(velocity_id));
    if (vVelocity) {
  ...
  if (ba_dyna_pt_obj_cam) {
    for (vector<MapPoint *>::iterator lit = lLocalMapPoints.begin(),
      ...
      pMP->mWorldPos_latestKF = Converter::toCvMat(
          belongedobj->pose_Twc_latestKF.pose * (vPoint->estimate()));
...
void Optimizer::OptimizeEssentialGraph(
  ...
  for (size_t i = 0, iend = vpKFs.size(); i < iend; i++) {
    ...
    if (pKF->mnId > pCurKF->mnId) { // lin755
      continue;
    }
  ...
  for (map<KeyFrame *, set<KeyFrame *>>::const_iterator
    ...
    for (set<KeyFrame *>::const_iterator sit = spConnections.begin(),
      ...
      e->setMeasurement(
          Sji); // from vertex i (first one)  to vertex j (second one).
