...
void Map::AddKeyFrame(KeyFrame *pKF) {
  ...
  for (std::set<KeyFrame *>::iterator iter = mspKeyFrames.begin();
       iter != mspKeyFrames.end(); iter++) {
    if ((*iter)->mnId == pKF->mnId) {
      return;
    }
  }
  mspKeyFrames.insert(pKF);

  if (pKF->mnId >= mnMaxKFid) // by me >  changes to >=    otherwise first
                              // keyframe is not reflected.
  {
    mnMaxKFid = pKF->mnId;
    mLatestKF = pKF;
  }
...
void Map::EraseKeyFrame(KeyFrame *pKF) {
  unique_lock<mutex> lock(mMutexMap);
  mspKeyFrames.erase(pKF); // called in Keyframe.cc when pKF is set bad... map
                           // point usually also removed.
}
...
vector<MapObject *> Map::GetGoodMapObjects() {
  ...
  for (set<MapObject *>::iterator sit = mspMapObjects.begin(),
                                  send = mspMapObjects.end();
       sit != send; sit++)
    if ((*sit)->isGood)
      res.push_back(*sit);
  return res;
