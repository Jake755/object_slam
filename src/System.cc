...
System::System(const string &strVocFile, const string &strSettingsFile, const string &strYoloModelPath, const eSensor sensor, bool use_loop_closing) : mSensor(sensor), use_loop_close(use_loop_closing), mbReset(false), mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false)
{
    ...
    //Load ORB Vocabulary
    cout << endl
         << "Loading ORB Vocabulary. This could take a while. Path: " << strVocFile << endl;
    ...
    mpTracker = new Tracking(this, mpVocabulary, mpFrameDrawer, mpMapDrawer,
                             mpMap, mpKeyFrameDatabase, strSettingsFile, strYoloModelPath, mSensor); // setting file read by tracker, not by mapper!

    if(whether_detect_object){
        mptDetectCuboid=new thread(&ORB_SLAM2::Tracking::run_detectObj, mpTracker);
    }

    //Initialize the Local Mapping thread and launch
    mpLocalMapper = new LocalMapping(mpMap, mSensor == MONOCULAR);
    mpLocalMapperLater = new LocalMappingLater(mpMap, mSensor == MONOCULAR);
    if (parallel_mapping){
    ...
    mpTracker->SetLocalMapperLater(mpLocalMapperLater);
...
void System::SaveTrajectoryKITTI(const string &filename)
{
    ...
    for (vector<cv::Mat>::iterator lit = mpTracker->mlRelativeFramePoses.begin(), lend = mpTracker->mlRelativeFramePoses.end(); lit != lend; lit++, lRit++, lT++)
    {
        ...
        if(pKF->mnFrameId > rawid){
            rawid=pKF->mnFrameId;
            id=rawid;
        }else if(pKF->mnFrameId < rawid){
            id++;
            rawid=id;
        }else if(pKF->mnFrameId == rawid ){
            id++;
        ...
        //   id << " " <<
        f << id << " " << setprecision(9) << Rwc.at<float>(0, 0) << " " << Rwc.at<float>(0, 1) << " " << Rwc.at<float>(0, 2) << " " << twc.at<float>(0) << " " << Rwc.at<float>(1, 0) << " " << Rwc.at<float>(1, 1) << " " << Rwc.at<float>(1, 2) << " " << twc.at<float>(1) << " " << Rwc.at<float>(2, 0) << " " << Rwc.at<float>(2, 1) << " " << Rwc.at<float>(2, 2) << " " << twc.at<float>(2) << endl;
    }
