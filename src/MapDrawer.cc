...
void MapDrawer::DrawMapPoints() {
  ...
  const vector<MapPoint *> &vpRefMPs =
      mpMap->GetReferenceMapPoints(); // local covisible map points. a subset of
                                      // vpMPs
  ...
  if (draw_nonlocal_mappoint) {
    ...
    for (size_t i = 0, iend = vpMPs.size(); i < iend; i++) {
      ...
      glVertex3f(pos.at<float>(0), pos.at<float>(1),
                 pos.at<float>(2)); //+shift_truth_map
    }
    glEnd();
  }
  ...
  if (1) // a subset of mappoints  points BA in local map
  {
    glPointSize(mPointSize * normal_point_size);
    glBegin(GL_POINTS);
    glColor3f(0.5, 0.5, 0.5); // darkgray
  ...
  if (enable_ground_height_scale) // draw ground point, better to show gray
                                  // points for above.
  {
  ...
  if (whether_detect_object && associate_point_with_object) {
    ...
    for (size_t object_id = 0; object_id < all_Map_objs.size(); object_id++) {
      ...
      owned_mappoints =
          obj_landmark->used_points_in_BA_filtered; // points really used in BA
      ...
      //			glColor4f(box_color(0), box_color(1),
      // box_color(2), 1.0f);
      glColor3f(1, 0, 0);
...
void MapDrawer::DrawMapCuboids() // ideally this should be draw map cuboids.
{
  ...
  const vector<MapObject *> all_Map_objs = mpMap->GetAllMapObjects();
  Vector4d front_face_color(1.0, 0.0, 1.0,
                            1.0); // draw front face edges magenta
  ...
  for (size_t object_id = 0; object_id < all_Map_objs.size(); object_id++) {
    ...
    // show objects that being optimized! for kitti fix scale, this will make
    // map visualization better.
    if (bundle_object_opti) {
      if (!obj_landmark->obj_been_optimized) {
        continue;
      }
    ...
    if (bundle_object_opti && whether_dynamic_object)
      cube_corners =
          obj_landmark->pose_Twc_afterba
              .compute3D_BoxCorner(); // show pose after BA, will have some
    ...
    // draw cuboid
    Vector3f box_color = box_colors[obj_landmark->mnId % box_colors.size()];
    glColor4f(box_color(0), box_color(1), box_color(2),
              1.0f); // draw all edges  cyan
    ...
    if (whether_dynamic_object && obj_landmark->is_dynamic &&
      ...
      for (auto it = obj_landmark->allDynamicPoses.begin();
        ...
        glVertex3f(cubepose.pose.translation()(0),
                   cubepose.pose.translation()(1),
...
void MapDrawer::DrawKeyFrames(const bool bDrawKF, const bool bDrawGraph) {
  ...
  if (bDrawKF) {
    for (size_t i = 0; i < vpKFs.size(); i++) {
      ...
      if (pKF->inLocalMappingLater == false) {
  ...
  if (draw_map_truth_paths)
    if (truth_poses.rows() > 0) {
      ...
      for (int pt_id = 0; pt_id < truth_poses.rows(); pt_id++) {
        glVertex3f(truth_poses(pt_id, 0), truth_poses(pt_id, 1),
                   truth_poses(pt_id, 2));
      }
      glEnd();
    }
    ...
    for (size_t i = 0; i < vpKFSequ.size(); i++) {
      ...
      if (currKf)
        if (!currKf->isBad()) {
          ...
          glVertex3f(currOw.at<float>(0), currOw.at<float>(1),
                     currOw.at<float>(2));
        }
    }
  glEnd();
