...
void Viewer::Run() {
  ...
  pangolin::Var<bool> menuFollowCamera(
      "menu.Follow Camera", false, true); // NOTE first value is default value
  ...
  if (build_worldframe_on_ground) // better to click followcamera!
    s_cam.SetModelViewMatrix(pangolin::ModelViewLookAt(
        0, mViewpointZ, -mViewpointY, 0, 0, 0, 0.0, 0, 1.0));
  ...
  while (1) {
    ...
    if (enable_viewimage) {
      ...
      // lin755
      cv::Mat imLater = mpFrameDrawer->DrawFrameLater();
      cv::imshow("Object SLAM: Later KFrame", imLater);
      cv::waitKey(mT);
    }
