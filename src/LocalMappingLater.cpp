...
LocalMappingLater::LocalMappingLater(Map *pMap, const float bMonocular)
...
void LocalMappingLater::SetLoopCloser(LoopClosing *pLoopCloser) {
  mpLoopCloser = pLoopCloser;
}

void LocalMappingLater::SetTracker(Tracking *pTracker) { mpTracker = pTracker; }

void LocalMappingLater::bundle_adjustment_caller() {
  if (whether_detect_object && bundle_object_opti) {
    ca::Profiler::tictoc("LBA");
    if (whether_dynamic_object)
      Optimizer::LocalBACameraPointObjectsDynamic(mpCurrentKeyFrame, &mbAbortBA,
                                                  mpMap, false, false);
    else
      Optimizer::LocalBACameraPointObjects(mpCurrentKeyFrame, &mbAbortBA, mpMap,
                                           false, false);
    ca::Profiler::tictoc("LBA");
  } else {
...
void LocalMappingLater::bundle_adjustment_caller_obj() {
  if (whether_detect_object && bundle_object_opti) {
    ca::Profiler::tictoc("LBA");
    bool flag = false;
    if (whether_dynamic_object)
      Optimizer::LocalBACameraPointObjectsDynamic(mpCurrentKeyFrame, &flag,
                                                  mpMap, false, false);
    else
      Optimizer::LocalBACameraPointObjects(mpCurrentKeyFrame, &flag, mpMap,
                                           false, false);
    ca::Profiler::tictoc("LBA");
  } else {
    ca::Profiler::tictoc("Normal BA");
...
bool LocalMappingLater::RunMappingIteration() {
  //     Tracking will see that Local Mapping is busy   if BA processing KF now,
  //     don't accept new KF
  SetAcceptKeyFrames(false);

  // Check if there are keyframes in the queue
  if (CheckNewKeyFrames()) // CheckNewKeyFrames()
  {
    ProcessNewKeyFrame(); // BoW conversion and insertion in Map

    // Check recent MapPoints
    MapPointCulling();

    // Triangulate new MapPoints, will update some point matches for some older
    // keyframes.
    CreateNewMapPoints();
    ...
    mpCurrentKeyFrame->addObjObNums(); // 添加物体观察数

    mbAbortBA = false;

    if (!CheckNewKeyFrames() &&
        !stopRequested()) // after all keyframes being inserted.
                          // 并且该关键帧的物体已经建好
    {
      // Local BA
      if (mpMap->KeyFramesInMap() > 2) { // mpMap->KeyFramesInMap() > 2
        // don't immediately BA if first frame is depth intialized, only a few
        // frames, baseline is small, will damage the map.
        if ((mbMonocular) &&
            (mono_firstframe_truth_depth_init ||
             mono_firstframe_Obj_depth_init) &&
      ...
      // Check redundant local Keyframes, could delete frame is many points in
      // this frame are observed before
      KeyFrameCulling();
    }
...
void LocalMappingLater::InsertKeyFrame(KeyFrame *pKF) {
  unique_lock<mutex> lock(mMutexNewKFs);
  mlNewKeyFrames.push_back(pKF);
  mbAbortBA = true; // lin755
}

bool LocalMappingLater::CheckNewKeyFrames() {
  unique_lock<mutex> lock(mMutexNewKFs);
  return (!mlNewKeyFrames.empty());
}

void LocalMappingLater::ProcessNewKeyFrame() {
  {
  ...
  //        mlpRecentAddedMapPoints=mpCurrentKeyFrame->mlpRecentAddedMapPoints;
  for (std::list<MapPoint *>::iterator iter =
           mpCurrentKeyFrame->mlpRecentAddedMapPoints.begin();
       iter != mpCurrentKeyFrame->mlpRecentAddedMapPoints.end(); iter++) {
    if ((*iter) == nullptr) {
      //                cout<<"mlpRecentAddedMapPoints nullptr"<<endl;
      continue;
    }
    if ((*iter)->isBad()) {
      //                cout<<"mlpRecentAddedMapPoints bad!"<<endl;
      continue;
    }
    mlpRecentAddedMapPoints.push_back(*iter);
  }
  std::cout << "[Later] Mapping: recent buffer pts:  "
            << mlpRecentAddedMapPoints.size() << endl;
  //        mpCurrentKeyFrame->addObjObNums(); // 添加物体观察数
       ...
       i++) // add frame observation for associated map points
  {
    ...
    // in mono/stereo/RGBD depth initialization, or stereo/RGBG create new KF,
    // new created map points already added to observation in tracking, there is
    // also matched points to previous frame, not yet add to observation
  ...
  if (use_dynamic_klt_features) { // false
...
void LocalMappingLater::MapPointCulling() {
  ...
  if (mbMonocular) {
    nThObs = 2;
    // my new added point from Monocular depth initilalization only has one
    // observation, should not delete
  ...
  while (lit != mlpRecentAddedMapPoints.end()) {
    MapPoint *pMP = *lit;
    if (pMP == nullptr) {
      std::cout << "bug found. LocalMappintLater line 318." << std::endl;
      continue;
    }
...
// Fisher-Yates shuffle    only the first num_random of the output vector is
// shuffled
...
void LocalMappingLater::CreateNewMapPoints() {
  ...
  for (size_t i = 0; i < vpNeighKFs.size(); i++) {
    ...
    for (int ikp = 0; ikp < nmatches; ikp++) {
      ...
      const float cosParallaxRays =
          ray1.dot(ray2) /
          (cv::norm(ray1) *
           cv::norm(ray2)); // parallax in keyframe 1 and keyframe 2
      ...
      if (cosParallaxRays < cosParallaxStereo && cosParallaxRays > 0 &&
        ...
        x3D = mpCurrentKeyFrame->UnprojectStereo(
            idx1); // create this keypoint from KF 1
  ...
  std::cout << "[Later] New Triangulated pt num:   " << nnew << std::endl;
  ...
  // NOTE doesn't change tracker->mCurrentFrame, not pointer!   Keyframe is
  // shared.
...
void LocalMappingLater::SearchInNeighbors() {
  ...
  for (vector<KeyFrame *>::const_iterator vit = vpNeighKFs.begin(),
    ...
    KeyFrame *pKFi = *vit;
    if (pKFi->isBad() ||
        pKFi->mnFuseTargetForKF >= mpCurrentKeyFrame->mnId) // lin755
      continue;
    ...
    for (vector<KeyFrame *>::const_iterator vit2 = vpSecondNeighKFs.begin(),
      ...
      if (pKFi2->isBad() ||
          ...
          pKFi2->mnId >= mpCurrentKeyFrame->mnId) // lin755
        continue;
  ...
  for (vector<KeyFrame *>::iterator vit = vpTargetKFs.begin(),
       ...
       vit != vend; vit++) {
    KeyFrame *pKFi = *vit;
    if (pKFi->isBad() || pKFi->mnId >= mpCurrentKeyFrame->mnId) { // lin755
      continue;
    }
    matcher.Fuse(
        pKFi,
        vpMapPointMatches); // merge close points, with similar descriptor.
  }
  ...
  for (vector<KeyFrame *>::iterator vitKF = vpTargetKFs.begin(),
    ...
    for (vector<MapPoint *>::iterator vitMP = vpMapPointsKFi.begin(),
      ...
      if (!pMP)
        continue;
      if (pMP->isBad() ||
          pMP->mnFuseCandidateForKF >= mpCurrentKeyFrame->mnId) // lin755
        continue;
      if (pMP->is_dynamic)
        continue;
...
cv::Mat LocalMappingLater::ComputeF12(KeyFrame *&pKF1, KeyFrame *&pKF2) {
...
void LocalMappingLater::RequestStop() {
...
bool LocalMappingLater::isStopped() {
  unique_lock<mutex> lock(mMutexStop);
  return mbStopped;
}

bool LocalMappingLater::stopRequested() {
  unique_lock<mutex> lock(mMutexStop);
  return mbStopRequested;
}

void LocalMappingLater::Release() {
  unique_lock<mutex> lock(mMutexStop);
...
bool LocalMappingLater::AcceptKeyFrames() {
  unique_lock<mutex> lock(mMutexAccept);
  return mbAcceptKeyFrames;
}

void LocalMappingLater::SetAcceptKeyFrames(bool flag) {
  unique_lock<mutex> lock(mMutexAccept);
  mbAcceptKeyFrames = flag;
}

bool LocalMappingLater::SetNotStop(bool flag) {
  unique_lock<mutex> lock(mMutexStop);
...
void LocalMappingLater::InterruptBA() { mbAbortBA = true; }

void LocalMappingLater::KeyFrameCulling() {
  ...
  for (vector<KeyFrame *>::iterator vit = vpLocalKeyFrames.begin(),
       ...
       vit != vend; vit++) {
    KeyFrame *pKF = *vit;
    if (pKF->mnId == 0 || pKF->mnId >= mpCurrentKeyFrame->mnId) // lin755
      continue;
...
cv::Mat LocalMappingLater::SkewSymmetricMatrix(const cv::Mat &v) {
  return (cv::Mat_<float>(3, 3) << 0, -v.at<float>(2), v.at<float>(1),
          v.at<float>(2), 0, -v.at<float>(0), -v.at<float>(1), v.at<float>(0),
          0);
}

void LocalMappingLater::RequestReset() {
  {
...
void LocalMappingLater::ResetIfRequested() {
  unique_lock<mutex> lock(mMutexReset);
  if (mbResetRequested) {
    mlNewKeyFrames.clear();
...
void LocalMappingLater::RequestFinish() {
  unique_lock<mutex> lock(mMutexFinish);
  mbFinishRequested = true;
}

bool LocalMappingLater::CheckFinish() {
  unique_lock<mutex> lock(mMutexFinish);
  return mbFinishRequested;
}

void LocalMappingLater::SetFinish() {
  unique_lock<mutex> lock(mMutexFinish);
  mbFinished = true;
...
bool LocalMappingLater::isFinished() {
  unique_lock<mutex> lock(mMutexFinish);
  return mbFinished;
}
