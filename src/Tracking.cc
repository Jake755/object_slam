...
Tracking::Tracking(System *pSys, ORBVocabulary *pVoc, FrameDrawer *pFrameDrawer,
                   ...
                   const string &strSettingPath, const string &strYoloModelPath,
                   const int sensor)
  ...
  // set initial camera pose wrt ground. by default camera parallel to ground,
  // height=1.7 (kitti)
  ...
  Eigen::Matrix3f rot =
      pose_quat.toRotationMatrix(); // 	The quaternion is required to be
                                    // normalized
  ...
  n.param<bool>("build_worldframe_on_ground", build_worldframe_on_ground,
                false); // transform initial pose and map to ground frame
  ...
  if (whether_detect_object) {
    ...
    if (!whether_read_offline_cuboidtxt) {
      ...
      detect_cuboid_obj->whether_sample_bbox_height = false; // lin755
      detect_cuboid_obj->whether_sample_cam_roll_pitch = false;
      detect_cuboid_obj->set_calibration(Kalib);
      yolact_obj = new yolact(strYoloModelPath);
    }

    if (!whether_read_offline_cuboidtxt) {
      n.param<bool>("whether_save_online_detected_cuboids",
                    whether_save_online_detected_cuboids, false);
      if (whether_save_online_detected_cuboids) {
        std::string save_object_pose_txt =
            base_data_folder + "/Outputs/orb_live_pred_objs_temp.txt";
  ...
  if (whether_detect_object) {
    n.param<bool>("whether_save_final_optimized_cuboids",
                  whether_save_final_optimized_cuboids, false);
  ...
  if (read_all_number_txt(truth_pose_txts, truth_cam_poses)) {
    ...
    for (int i = 0; i < mpMapDrawer->truth_poses.rows(); i++) {
      mpMapDrawer->truth_poses.row(i) =
          truth_cam_poses.row(i * 10).segment(1, 3);
      if (build_worldframe_on_ground) {
        Eigen::Quaterniond pose_quat(
            truth_cam_poses(i * 10, 7), truth_cam_poses(i * 10, 4),
        ...
        pose_to_init.col(3).head<3>() = Eigen::Vector3d(
            truth_cam_poses(i * 10, 1), truth_cam_poses(i * 10, 2),
            truth_cam_poses(i * 10, 3));
        Eigen::Matrix4d pose_to_ground =
            InitToGround_eigen.cast<double>() * pose_to_init;
...
void Tracking::SetLocalMapperLater(LocalMappingLater *pLocalMapperLater) {
  mpLocalMapperLater = pLocalMapperLater;
}
...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp,
  ...
  if (mState == NOT_INITIALIZED || mState == NO_IMAGES_YET) {
    if ((!mono_firstframe_truth_depth_init) || (mCurrentFrame.mnId > 0)) {
      mCurrentFrame = Frame(mImGray, timestamp, mpIniORBextractor,
                            mpORBVocabulary, mK, mDistCoef, mbf, mThDepth);
    } else { // read (truth) depth /stereo image for first frame
      // not good to read first frame's predicted depth by object. quite
      // inaccurate. orb slam will create left/right coordinates based on that,
      // and will be used for optimizer.
      ...
      if (!right_stereo_img.data)
        ROS_ERROR_STREAM("Cannot read first stereo file  "
                         << right_kitti_img_file);
      else
        ROS_WARN_STREAM("Read first right stereo size  "
                        << right_stereo_img.rows);
      std::cout << "Read first right depth size  " << right_stereo_img.rows
                << "  baseline  " << mbf << std::endl;
      mCurrentFrame = Frame(mImGray, right_stereo_img, timestamp,
  ...
  if (all_offline_object_cubes.size() > 0) {
    if ((mCurrentFrame.mnId > 0) &&
      ...
      if (int(mCurrentFrame.mnId) !=
          (msg_seq_id - start_msg_seq_id)) // can use frame->IdinRawImages =
                                           // msg_seq_id-start_msg_seq_id  need
                                           // to change lots of stuff.
      {
        ROS_ERROR_STREAM(
            "Different frame ID, might due to lost frame from bag.   "
  ...
  if (whether_detect_object) {
    mCurrentFrame.raw_img = mImGray;    // I clone in Keyframe.cc  don't need to
                                        // clone here. but do not delete.
    mCurrentFrame.raw_color = mImColor; // lin755
  }
...
void Tracking::Track() {
  ...
  if (parallel_mapping) // if sequential mapping, no need to use mutex,
                        // otherwise conflict with localmapping optimizers.
    unique_lock<mutex> lock(mpMap->mMutexMapUpdate);

  if (mState == NOT_INITIALIZED) // initialization
  {
    if (mSensor == System::STEREO || mSensor == System::RGBD)
      StereoInitialization(); // for stereo or RGBD, the first frame is used to
                              // initialize the keyframe and map
    else {
      bool special_initialization = false;
      if (mCurrentFrame.mnId == 0) {
        if (mono_firstframe_truth_depth_init) {
          special_initialization = true;
          StereoInitialization(); // if first frame has truth depth, we can
                                  // initialize simiar to stereo/rgbd. create
                                  // keyframe for it.
        } else if (mono_firstframe_Obj_depth_init) {
          special_initialization = true;
          // similar to stereo initialization, but directly create map point.
          // don't create stereo right coordinate have less effect on g2o
          // optimization.  because depth initialization is not accurate
          MonoObjDepthInitialization();
        }
      }
      if (!special_initialization)
        MonocularInitialization(); // usually for monocular, need to wait for
                                   // several frames, with enough parallax
    }
    ...
    if (!mbOnlyTracking) {
      ...
      // explicitly activate the "only tracking" mode.
      if (mState == OK) {
        ...
        if (mVelocity.empty() ||
            mCurrentFrame.mnId <
                mnLastRelocFrameId +
                    2) // at initialization or relocalization stage
        {
          bOK = TrackReferenceKeyFrame();
        } else {
          //                    TrackWithMotionModel()"<<endl;
          bOK = TrackWithMotionModel(); // usual way of camera tracking
          if (!bOK) // if trackMotion model failed,   usually not happening
          {
            bOK = TrackReferenceKeyFrame();
            if (!bOK)
              std::cout << "[ERROR] Not OK right after "
                           "TrackReferenceKeyFrame!!!"
                        << std::endl;
          }
        }
      } else {
        ROS_ERROR_STREAM(" Last frame tracking bad, start "
                         "relocalization!!");
        bOK = Relocalization();
      }
    } else // Only Tracking: Local Mapping is deactivated, usually not
           // happening.
    {
      if (mState == LOST) {
        bOK = Relocalization();
    ...
    if (!mbOnlyTracking) // usually the case
    {
      if (bOK) {
        bOK = TrackLocalMap();
        if (!bOK)
          std::cout << "[WARN] Not OK right after tracking local map!!!"
                    << std::endl;
      }
    } else {
    ...
    if (bOK) {
      ...
      // Clean temporal point matches  for rgbd/stereo directly inited map
      // point. not monoculari
      for (int i = 0; i < mCurrentFrame.N; i++) {
        ...
        if (pMP)
          if (pMP->Observations() < 1) {
            ...
            mCurrentFrame.mvpMapPoints[i] =
                static_cast<MapPoint *>(NULL); // lin755
          }
      }
      ...
      if (NeedNewKeyFrame()) {
        CreateNewKeyFrame(); // push into the localmapping buffer. mapping
                             // thread will BA.
        created_keyframe = true;
        // ROS_WARN_STREAM("Created new keyframe!   " << mpReferenceKF->mnId <<
        // "   total ID  " << mpReferenceKF->mnFrameId);
      }

      // TODO DONE. 绘制2D图可以同时绘制线程中的检测框
      mpFrameDrawer->Update(
          this); // I put here so that frame drawer syncs with new keyframe.
    ...
    if (mState == LOST) {
      if (mpMap->KeyFramesInMap() <= 5) {
        cout << "[ERROR] Track lost soon after "
                "initialisation, reseting..."
             << endl;
        mpSystem->Reset();
        return;
      }
  ...
  if (whether_save_final_optimized_cuboids) {
    if ((mCurrentFrame.mnId >= final_object_record_frame_ind) &&
        (!done_save_obj_to_txt)) {
  ...
  // implement blocking    block tracking thread, wait until mapping has
  // finished (refine KF)  then track new frame.
  if (!parallel_mapping) {
...
void Tracking::StereoInitialization() {
  ...
  if (mCurrentFrame.N > 500) {
    ...
    if (build_worldframe_on_ground) // transform initial pose and map to ground
                                    // frame
    ...
    ROS_WARN_STREAM("Created new keyframe! Stereo Init   "
    ...
    for (int i = 0; i < mCurrentFrame.N; i++) {
      ...
      if (z > 0) // monocular has negative value
      {
        ...
        MapPoint *pNewMP =
            new MapPoint(x3D, pKFini, mpMap); // initialize new map points
    ...
    cout << "Not enough points for initialization  " << mCurrentFrame.N
         << std::endl;
}
...
void Tracking::MonoObjDepthInitialization() {
  ...
  if (mCurrentFrame.N > 500) {
    ...
    //		{
    //			DetectCuboid(pKFini);
    //			AssociateCuboids(pKFini); // associate cuboids.  //
    // first frame don't need mpLastKeyFrame as I check .mID==0
    //		}
    ...
    for (int i = 0; i < mCurrentFrame.N; i++) {
      ...
      if (whether_detect_object && mono_firstframe_Obj_depth_init &&
        ...
        if (id > -1) {
          point_depth =
              pKFini->local_cuboids[id]->cube_meas.translation()[2]; // camera z
      ...
      if (point_depth > 0) {
        MapPoint *pNewMP = new MapPoint(
            x3D, pKFini, mpMap); // x3d already world frame based on pose
    ...
    if (mpMap->MapPointsInMap() == 0) {
      ROS_ERROR_STREAM(
          "Bad MonoObjDepthInitialization! No map points! Break systems!");
      exit(0);
    }
  ...
  if (!mpInitializer) {
    ...
    if (mCurrentFrame.mvKeys.size() > 100) {
      ...
      for (size_t i = 0; i < mCurrentFrame.mvKeysUn.size(); i++)
        mvbPrevMatched[i] = mCurrentFrame.mvKeysUn[i].pt; //像素点
...
void Tracking::CreateInitialMapMonocular() {
  ...
  Optimizer::GlobalBundleAdjustemnt(
      mpMap, 20); // TODO whether add object in it... might also make the
                  // initialization fail????
  ...
  // Scale points. map points are in the first frame, which is world frame. so
  // we can directly scale it
                                  ...
                                  // frame
  {
    ...
    for (size_t iMP = 0; iMP < vpAllMapPoints.size(); iMP++) {
      if (vpAllMapPoints[iMP]) {
        MapPoint *pMP = vpAllMapPoints[iMP];
        pMP->SetWorldPos(InitToGround.rowRange(0, 3).colRange(0, 3) *
                             pMP->GetWorldPos() +
    ...
    if (mCurrentFrame.mvpMapPoints[i]) {
      if (mCurrentFrame.mvbOutlier[i]) {
        ...
        if (!(whether_dynamic_object &&
              mCurrentFrame.mvpMapPoints[i]->is_dynamic))
          nmatchesMap++;
    }
  }
...
bool Tracking::TrackWithMotionModel() {
  ...
  // set all matched map points to null, as this is first time to match with map
  // points.
  ...
  int nmatches =
      matcher.SearchByProjection(mCurrentFrame, mLastFrame, searchRadiusFactor,
  ...
  if (nmatches < 20) {
    ...
    nmatches = matcher.SearchByProjection(mCurrentFrame, mLastFrame,
                                          2 * searchRadiusFactor,
                                          mSensor == System::MONOCULAR);
  }

  if (whether_detect_object) {
    if (use_dynamic_klt_features)
      matcher.SearchByTrackingHarris(mCurrentFrame, mLastFrame,
                                     searchRadiusFactor,
                                     mSensor == System::MONOCULAR);
    else
...
bool Tracking::TrackLocalMap() {
  ...
  if (whether_detect_object) // for dynamic scenarios, there might not be enough
                             // feature matches
    map_match_thres = 20;
  ...
  if (mCurrentFrame.mnId < mnLastRelocFrameId + mMaxFrames &&
      mnMatchesInliers < map_match_thres)
    return false;
...
bool Tracking::NeedNewKeyFrame() {
  ...
  if (mpLocalMapper->isStopped() || mpLocalMapper->stopRequested() ||
      mpLocalMapperLater->isStopped() || mpLocalMapperLater->stopRequested())
    return false;
  ...
  if (nKFs <= 2)
    nMinObs = 2;
  // if first frame depth initialized by object... not actual measurement. map
  // point only have one observation need to reduce the threshold for
  // initialization.
  ...
  int nRefMatches = mpReferenceKF->TrackedMapPoints(
      nMinObs); // tracked mappoints with at least nMinObs in reference Key
                // frame

  // whether Local Mapping accept keyframes   during mapping iteration, don't
  // allow to accept keyframes
  bool bLocalMappingIdle =
      mpLocalMapper->AcceptKeyFrames() && mpLocalMapperLater->AcceptKeyFrames();
  ...
  if (mSensor != System::MONOCULAR) {
    for (int i = 0; i < mCurrentFrame.N; i++) {
      if (mCurrentFrame.mvDepth[i] > 0 && mCurrentFrame.mvDepth[i] < mThDepth) {
        nTotal++;
        if (mCurrentFrame
                .mvpMapPoints[i]) // if already associated a map point, done in
  ...
  if ((c1a || c1b || c1c) && c2) {
    ...
    if (bLocalMappingIdle) {
      ...
      mpLocalMapperLater->InterruptBA();
      if (mSensor != System::MONOCULAR) {
...
void Tracking::DetectCuboid(KeyFrame *pKF) {
  cv::Mat pop_pose_to_ground; // pop frame pose to ground frame.  for offline
                              // txt, usually local ground.  for online detect,
                              // usually init ground.
  std::vector<ObjectSet>
      all_obj_cubes; // in ground frame, no matter read or online detect
  std::vector<Vector4d> all_obj2d_bbox;
  std::vector<double> all_box_confidence;
  std::vector<int> all_box_labels; // lin755
  std::vector<cv::Mat> obj_masks;
  vector<int> truth_tracklet_ids;

  if (whether_read_offline_cuboidtxt) // saved object txt usually is usually
                                      // poped in local ground frame, not the
                                      // global ground frame.
  {
    if (all_offline_object_cubes.size() == 0)
      return;
    pop_pose_to_ground =
        InitToGround.clone(); // for kitti, I used InitToGround to pop offline.
    ...
    for (int i = 0; i < pred_frame_objects.rows(); i++) {
      ...
      raw_cuboid->scale =
          Vector3d(pred_frame_objects(i, 4), pred_frame_objects(i, 5),
                   pred_frame_objects(i, 6));
      raw_cuboid->rect_detect_2d = pred_frame_objects.row(i).segment<4>(7);
      raw_cuboid->box_config_type = Vector2d(
          1, 1); // randomly given unless provided. for latter visualization
    ...
    //		std::string data_edge_data_dir = base_data_folder +
    //"/edge_detection/LSD/"; 		std::string data_yolo_obj_dir =
    // base_data_folder +
    //"/mats/filter_match_2d_boxes_txts/"; 		char frame_index_c[256];
    //		sprintf(frame_index_c, "%04d", (int)pKF->mnFrameId); // format
    // into 4 digit
    //
    //		// read detected edges
    //		Eigen::MatrixXd all_lines_raw(100, 4); // 100 is some large
    // frame number,   the txt edge index start from 0
    //		read_all_number_txt(data_edge_data_dir + frame_index_c +
    //"_edge.txt", all_lines_raw);
    //
    ...
    //		if (!read_obj_detection_txt(data_yolo_obj_dir + frame_index_c +
    // obj_2d_txt_postfix, raw_all_obj2d_bbox, object_classes))
    //			ROS_ERROR_STREAM("Cannot read yolo txt  " <<
    // data_yolo_obj_dir
    //+ frame_index_c + obj_2d_txt_postfix);

    // ----------------- lin755 ----------------------//
    //        std::vector<yolact::Object> yolact_2d_objs;
    pKF->local_cuboids.clear();
    pKF->yolact_objs.clear();
    pKF->yolact_objs =
        yolact_obj->yolactRun(pKF->raw_color, pKF->mnId); // 检测2d对象框
    Eigen::MatrixXd raw_all_obj2d_bbox(10, 6);
    for (int i = 0; i < pKF->yolact_objs.size(); i++) {
      raw_all_obj2d_bbox(i, 0) = pKF->yolact_objs[i].rect.x;
      raw_all_obj2d_bbox(i, 1) = pKF->yolact_objs[i].rect.y;
      raw_all_obj2d_bbox(i, 2) = pKF->yolact_objs[i].rect.width;
      raw_all_obj2d_bbox(i, 3) = pKF->yolact_objs[i].rect.height;
      raw_all_obj2d_bbox(i, 4) = pKF->yolact_objs[i].prob;
      raw_all_obj2d_bbox(i, 5) = pKF->yolact_objs[i].label;
    }

    Eigen::MatrixXd all_lines_raw(
        500,
        4); // 100 is some large frame number,   the txt edge index start from 0

    // remove some 2d boxes too close to boundary.
    int boundary_threshold = 20;
    int img_width = pKF->raw_color.cols;
    std::vector<int> good_object_ids;
    for (int i = 0; i < pKF->yolact_objs.size();
         i++) { // raw_all_obj2d_bbox.rows()
      //            cout<<"raw_all_obj2d_bbox(i, 0):"<<raw_all_obj2d_bbox(i,
      //            0)<<" "<<boundary_threshold<<endl;
      //            cout<<"(raw_all_obj2d_bbox(i, 2)"<<+ raw_all_obj2d_bbox(i,
      //            2)<<" "<<img_width - boundary_threshold<<endl;
      if ((raw_all_obj2d_bbox(i, 0) > boundary_threshold) &&
          (raw_all_obj2d_bbox(i, 0) + raw_all_obj2d_bbox(i, 2) <
           img_width - boundary_threshold)) {
        //                cout<<"good_object++ :"<<i<<endl;

        if (raw_all_obj2d_bbox(i, 5) == 3) { // 3 = car
          good_object_ids.push_back(i);
        }
        //                good_object_ids.push_back(i);
      }
    }
    cout << "检测物体数: " << good_object_ids.size() << endl;

    Eigen::MatrixXd all_obj2d_bbox_infov_mat(good_object_ids.size(), 6);
    for (size_t i = 0; i < good_object_ids.size(); i++) {
      all_obj2d_bbox_infov_mat.row(i) =
          raw_all_obj2d_bbox.row(good_object_ids[i]);
      all_obj2d_bbox.push_back(raw_all_obj2d_bbox.row(good_object_ids[i]));
      all_box_confidence.push_back(
          raw_all_obj2d_bbox(good_object_ids[i], 4)); // TODO DONE! change here.
      all_box_labels.push_back(raw_all_obj2d_bbox(good_object_ids[i], 5));
      obj_masks.push_back(pKF->yolact_objs[good_object_ids[i]].mask);
    }
    //        for(int i=0;i<all_obj2d_bbox.size();i++){
    //            cout<<"all_obj2d_bbox: " << all_obj2d_bbox[i];
    //            cout<<" all_box_confidence: " << all_box_confidence[i]<<endl;
    //        }
    cv::Mat frame_pose_to_init = pKF->GetPoseInverse(); // camera to init world
    //        Twc:"<<pKF->GetPoseInverse()<<endl;
    cv::Mat frame_pose_to_ground;
    if (build_worldframe_on_ground) { // true.  if initial world frame is on
                                      // ground, directly use it.
    ...
    Eigen::Matrix4f cam_transToGround =
        Converter::toMatrix4f(pop_pose_to_ground);
    cout << "cam_transToGround: " << cam_transToGround << endl;
    detect_cuboid_obj->detect_cuboid(
        pKF->raw_color, cam_transToGround.cast<double>(),
        all_obj2d_bbox_infov_mat, all_lines_raw, all_obj_cubes, obj_masks);
  }
  // copy and analyze results. change to g2o cuboid.
  g2o::SE3Quat frame_pose_to_init = Converter::toSE3Quat(
      pKF->GetPoseInverse()); // camera to init, not always ground.
  ...
  for (int ii = 0; ii < (int)all_obj_cubes.size(); ii++) {
    if (all_obj_cubes[ii].size() > 0) // if has detected 3d Cuboid
    {
      ...
      g2o::cuboid cube_ground_value; // offline cuboid txt in local ground
                                     // frame.  [x y z yaw l w h]
      Vector9d cube_pose;
      cube_pose << raw_cuboid->pos[0], raw_cuboid->pos[1], raw_cuboid->pos[2],
          0, 0, raw_cuboid->rotY, raw_cuboid->scale[0], raw_cuboid->scale[1],
          raw_cuboid->scale[2];
      ...
      g2o::cuboid cube_local_meas = cube_ground_value.transform_to(
          Converter::toSE3Quat(pop_pose_to_ground));
      newcuboid->label = all_box_labels[ii]; // lin755
      newcuboid->mask = obj_masks[ii];
      newcuboid->prob = all_box_confidence[ii];
      newcuboid->cube_meas = cube_local_meas;
      newcuboid->bbox_2d = cv::Rect(
          raw_cuboid->rect_detect_2d[0], raw_cuboid->rect_detect_2d[1],
          raw_cuboid->rect_detect_2d[2], raw_cuboid->rect_detect_2d[3]);
      newcuboid->bbox_vec = Vector4d(
          (double)newcuboid->bbox_2d.x + (double)newcuboid->bbox_2d.width / 2,
      ...
      newcuboid->bbox_2d_tight = cv::Rect(
          raw_cuboid->rect_detect_2d[0] + raw_cuboid->rect_detect_2d[2] / 10.0,
          raw_cuboid->rect_detect_2d[1] + raw_cuboid->rect_detect_2d[3] / 10.0,
          raw_cuboid->rect_detect_2d[2] * 0.8,
          raw_cuboid->rect_detect_2d[3] * 0.8);
      get_cuboid_draw_edge_markers(newcuboid->edge_markers,
                                   raw_cuboid->box_config_type, false);
      ...
      if (1) {
        double obj_cam_dist =
            std::min(std::max(newcuboid->cube_meas.translation()(2), 10.0),
                     30.0); // cut into [a,b]
      ...
      if (all_box_confidence[ii] > 0)
        newcuboid->meas_quality = all_box_confidence[ii]; // or = 原本:*=

      if (newcuboid->meas_quality < 0.1)
        ROS_WARN_STREAM(
            "Abnormal measure quality!!:   " << newcuboid->meas_quality);
      pKF->local_cuboids.push_back(newcuboid);
    }
  }

  //	std::cout << "Created local object num: " << pKF->local_cuboids.size()
  //<< std::endl; 	std::cout << "Detect cuboid for pKF id: " << pKF->mnId
  //<< " total id: " << pKF->mnFrameId << "  numObj: " <<
  // pKF->local_cuboids.size()
  //<< std::endl;
  std::cout << "关键帧 pKF id: " << pKF->mnId << " 全局id: " << pKF->mnFrameId
            << " 建成物体: " << pKF->local_cuboids.size() << std::endl;

  // lin755 savecuboids
  if (0) {
    char frame_index_c[256];
    stringstream ss;
    ss << setw(4) << setfill('0') << pKF->mnFrameId;
    string frame_index;
    ss >> frame_index;
    // str = ss.str();  //same
    string filename = "/home/lin755/catkin_cubeSLAM/src/cube_slam-master/"
                      "object_slam/data/pred_3d_obj_matched_txt/" +
                      frame_index + "_3d_cuboids.txt";
    ofstream f;
    f.open(filename.c_str());
    int prob = 1;
    for (int ii = 0; ii < (int)all_obj_cubes.size(); ii++) {
      if (all_obj_cubes[ii].size() >
          0) // if has detected 3d Cuboid, always true in this case
      {
        cuboid *raw_cuboid = all_obj_cubes[ii][0];
        f << raw_cuboid->pos[0] << " " << raw_cuboid->pos[1] << " "
          << raw_cuboid->pos[2] << " " << raw_cuboid->rotY << " "
          << raw_cuboid->scale[0] << " " << raw_cuboid->scale[1] << " "
          << raw_cuboid->scale[2] << " " << raw_cuboid->rect_detect_2d[0] << " "
          << raw_cuboid->rect_detect_2d[1] << " "
          << raw_cuboid->rect_detect_2d[2] << " "
          << raw_cuboid->rect_detect_2d[3] << " " << prob << endl;
      }
    }
    f.close();
  }
    ...
    for (int ii = 0; ii < (int)all_obj_cubes.size(); ii++) {
          ...
          0) // if has detected 3d Cuboid, always true in this case
      {
        ...
        cube_pose << raw_cuboid->pos[0], raw_cuboid->pos[1], raw_cuboid->pos[2],
            0, 0, raw_cuboid->rotY, raw_cuboid->scale[0], raw_cuboid->scale[1],
            raw_cuboid->scale[2];
        save_online_detected_cuboids << pKF->mnFrameId << "  "
                                     << cube_pose.transpose() << "\n";
      }
    }
  }
  ...
  if (associate_point_with_object) {
    if (!whether_dynamic_object) // for old non-dynamic object, associate based
                                 // on 2d overlap... could also use instance
                                 // segmentation
    {
      // 将关键帧的所有特征点取出
      pKF->keypoint_associate_objectID = vector<int>(pKF->mvKeys.size(), -1);
      // 遍历所有物体，若重叠部分超过15%则设置为TRUE
      // TODO lin755
        ...
        for (size_t i = 0; i < pKF->local_cuboids.size(); i++)
          if (!overlapped[i])
            for (size_t j = i + 1; j < pKF->local_cuboids.size(); j++)
              if (!overlapped[j]) {
                float iou_ratio =
                    bboxOverlapratio(pKF->local_cuboids[i]->bbox_2d,
      ...
      if (!enable_ground_height_scale)       // true
      {                                      // slightly faster
        if (pKF->local_cuboids.size() > 0) { // if there is object
          int all_associated_times = 0;
          for (size_t i = 0; i < pKF->mvKeys.size(); i++) {
            int associated_times = 0;
            for (size_t j = 0; j < pKF->local_cuboids.size(); j++) {
              if (!overlapped[j]) {
                if (pKF->local_cuboids[j]->mask.ptr<uchar>(
                        pKF->mvKeys[i].pt.y)[(int)pKF->mvKeys[i].pt.x] ==
                    255) // pKF->local_cuboids[j]->bbox_2d.contains(pKF->mvKeys[i].pt)
                {
                  associated_times++;
                  all_associated_times++;
          ...
          cout << "pKF id: " << pKF->mnId
               << " 物体关联特征点总数: " << all_associated_times << endl;
        }
      } else {
        // 取出所有特征点
        // 计算物体是否包含该点,如果包含且物体不与其他物体重合，关联次数增一
        int all_associated_times = 0;
        ...
        for (size_t i = 0; i < pKF->mvKeys.size(); i++) {
          ...
          for (size_t j = 0; j < pKF->local_cuboids.size(); j++) {
            if (pKF->local_cuboids[j]->mask.ptr<uchar>(
                    pKF->mvKeys[i].pt.y)[(int)pKF->mvKeys[i].pt.x] == 255) {
              pKF->local_cuboids[j]->mvkeys.push_back(
                  pKF->mvKeys[i]); // 将物体所属关键点加入到临时vector中
        ...
        if (height_esti_history.size() == 0) {
          pKF->local_cuboids
              .clear(); // don't do object when in initial stage...
          pKF->keypoint_associate_objectID.clear();
          cout << "[WARN] pKF->local_cuboids clear" << endl;
        }
      }
    }

    if (whether_dynamic_object) //  for dynamic object, I use instance
                                //  segmentation
    {
      if (pKF->local_cuboids.size() > 0) // if there is object
      {
  ...
  if (scene_unique_id == kitti) {
    if (mono_allframe_Obj_depth_init)
      object_own_point_threshold =
          50; // 50 using 10 is too noisy.... many objects don't have enough
              // points to match with others then will create as new...
    else
      object_own_point_threshold =
          30; // 30 if don't initialize object point sepratedly, there won't be
              // many points....  tried 20, not good...
  }
  ...
  if (use_truth_trackid) // very accurate, no need of object point for
                         // association
  ...
  // problems come
  // std::list<KeyFrame *> checkframes =
  //     localKeyFrames; // only check recent to save time
  // for (std::list<KeyFrame *>::iterator iter = localKeyFrames.begin();
  //      iter != localKeyFrames.end(); iter++) {
  //   KeyFrame *kfs = *iter;
  ...
  // std::vector<KeyFrame *> checkframes = mvpLocalKeyFrames; // only check
  // recent to save time
  std::vector<KeyFrame *> checkframes =
      LocalObjKeyFrames; // only check recent to save time
...
void Tracking::AssociateCuboids(KeyFrame *pKF) {
  // loop over current KF's objects, check with all past objects (or local
  // objects), compare the associated object map points. (if a local object is
  // not associated, could re-check here as frame-object-point might change
  // overtime, especially due to triangulation.)

  std::vector<MapObject *> LocalObjectsCandidates;
  std::vector<MapObject *> LocalObjectsLandmarks;
  // keypoint might not added to frame observation yet, so object might not have
  // many associated points yet.... method 1: just check current frame's object,
  // using existing map point associated objects. same as plane association,
  // don't just check current frame, but check all recent keyframe's unmatched
  // objects...
  // for (size_t i = 0; i < mvpLocalKeyFrames.size(); i++) // pKF is not in
  // mvpLocalKeyFrames yet
  //	{
  ...
  //			if (kfs->cuboids_landmark[j]) // might be deleted due to
  // badFlag() 				if (!kfs->cuboids_landmark[j]->isBad())
  // if (kfs->cuboids_landmark[j]->association_refid_in_tracking != pKF->mnId)
  // // could also use set to avoid duplicates
  //					{
  //						LocalObjectsLandmarks.push_back(kfs->cuboids_landmark[j]);
  //						kfs->cuboids_landmark[j]->association_refid_in_tracking
  //= pKF->mnId;
  //					}
  //	}
  for (size_t i = 0; i < LocalObjKeyFrames.size();
       i++) // pKF is not in mvpLocalKeyFrames yet
  {
    KeyFrame *kfs = LocalObjKeyFrames[i];
    // lin755>
    if (kfs->mnId > pKF->mnId) {
      continue;
    }
  ...
  std::cout << "物体关联 候选物体数:" << LocalObjectsCandidates.size()
            << " 建成物体数:" << LocalObjectsLandmarks.size() << std::endl;
  int largest_shared_num_points_thres = 10;
  if (mono_allframe_Obj_depth_init)
    largest_shared_num_points_thres = 20;
  if (scene_unique_id == kitti)
    largest_shared_num_points_thres =
        5; // lin755 10 kitti vehicle occupy large region

  if (whether_detect_object &&
      mono_allframe_Obj_depth_init) // dynamic object is more difficult.
                                    // especially reverse motion
  ...
  for (size_t i = 0; i < LocalObjectsCandidates.size(); i++) {
    ...
    std::vector<MapPoint *> object_owned_pts =
        candidateObject
            ->GetPotentialMapPoints(); //潜在点来自KeyFrame::addObjObNums()
    ...
    if (largest_shared_objectlandmark ==
        nullptr) // if not found, create as new landmark.  either using original
                 // local pointer, or initialize as new
    {
      if (use_truth_trackid) {
      ...
      candidateObject->already_associated =
          true; // must be put before SetAsLandmark();
      KeyFrame *refframe = candidateObject->GetReferenceKeyFrame();
      candidateObject->addObservation(
          refframe,
          candidateObject->object_id_in_localKF); // add to frame observation
      refframe->cuboids_landmark.push_back(candidateObject);
      candidateObject->mnId =
          MapObject::getIncrementedIndex(); // mpMap->MapObjectsInMap();  //
      ...
      if (scene_unique_id == kitti) // object scale change back and forth
      {
        ...
        candidateObject->allDynamicPoses[refframe] = make_pair(
            cubeglobalpose, false); // Vector6d::Zero()  false means not BAed
      }
    ...
    } else // if found, then update observation.
    {
      candidateObject->already_associated =
          true; // must be put before SetAsLandmark();
      KeyFrame *refframe = candidateObject->GetReferenceKeyFrame();
      largest_shared_objectlandmark->addObservation(
          refframe, candidateObject->object_id_in_localKF);
  ...
  if (remove_object_outlier) {
    ...
    for (size_t i = 0; i < all_objects.size(); i++) {
      ...
      if ((!pMObject->isBad()) &&
          (!pMObject->isGood)) // if not determined good or bad yet.
        if ((int)pMObject->GetLatestKeyFrame()->mnId <
            (int)pKF->mnId - 20) // 20
        {
          // if not recently observed, and not enough observations.  NOTE if
          // point-object not used in BA, filtered size will be zero...
          ...
          if (pMObject->Observations() < minimum_object_observation) {
            pMObject->SetBadFlag();
            cout << "[ERROR] BAD object! id: " << pMObject->mnId
                 << "  obs: " << pMObject->Observations() << "  BApoints: "
...
void Tracking::localKFListRemove(KeyFrame *pKF) {
  unique_lock<mutex> lock(mMutexLocalKFs);
  localKeyFrames.pop_front();
  // localKeyFrames.remove(pKF);
}

void Tracking::run_detectObj() {
  while (1) {
    if (whether_detect_object && localKeyFrames.size() > 0) {
      KeyFrame *pKF = nullptr;
      {
        unique_lock<mutex> lock(mMutexLocalKFs);
        pKF = localKeyFrames.front();
      }
      if (pKF == nullptr || pKF->isBad()) {
        localKFListRemove(pKF);
        continue;
      }
      LocalObjKeyFrames.push_back(pKF); //加入局部物体关联关键帧
      pKF->inLocalMappingLater = true;
      cout << "run_detectObj帧id: " << pKF->mnFrameId
           << " 关键帧总数: " << localKeyFrames.size() << endl;
      ca::Profiler::tictoc("Detect object");
      DetectCuboid(pKF);
      ca::Profiler::tictoc("Detect object");
      ca::Profiler::tictoc("Associate object");
      AssociateCuboids(pKF);
      ca::Profiler::tictoc("Associate object");
      mpFrameDrawer->UpdateLater(pKF, this); // 更新视图
      pKF->IsCreatedObjs = true;
      mpLocalMapperLater->InsertKeyFrame(pKF);
      mpLocalMapperLater->SetNotStop(false);
      mpLocalMapperLater->RunMappingIteration();

      // mpLocalMapper->bundle_adjustment_caller_obj(pKF);
      // TODO 绘制2D图可以同时绘制线程中的检测框
      // mpFrameDrawer->Update(this);
      // I put here so that frame drawer syncs with new keyframe.
      localKFListRemove(pKF);
      pKF->inLocalMappingLater = false;
      // checkValidLocalObjKFs();
    }
    usleep(100);
  }
}

void Tracking::checkValidLocalObjKFs() {
  for (std::vector<KeyFrame *>::iterator iter = LocalObjKeyFrames.begin();
       iter != LocalObjKeyFrames.end();) {
    int okNums = 0;
    for (size_t j = 0; j < (*iter)->local_cuboids.size(); j++) {
      MapObject *mPO = (*iter)->local_cuboids[j];
      if (mPO->isBad() || mPO->isGood) {
        okNums++;
      }
    }
    cout << "okNums: " << okNums
         << " local_cuboids:" << (*iter)->local_cuboids.size() << endl;
    if (okNums == (*iter)->local_cuboids.size()) {
      iter = LocalObjKeyFrames.erase(iter);
    } else {
      iter++;
    }
  }
...
void Tracking::CreateNewKeyFrame() {
  ...
  // if (!mpLocalMapperLater->SetNotStop(true))
  //     return;

  KeyFrame *pKF = new KeyFrame(mCurrentFrame, mpMap, mpKeyFrameDB);

  ROS_WARN_STREAM("Created new keyframe!   " << pKF->mnId << "   total ID  "
                                             << pKF->mnFrameId);
    ...
    if (!vDepthIdx.empty()) {
      ...
      for (size_t j = 0; j < vDepthIdx.size(); j++) {
        ...
        if (bCreateNew) {
          cv::Mat x3D = mCurrentFrame.UnprojectStereo(i);
          MapPoint *pNewMP = new MapPoint(
              x3D, pKF, mpMap); // also initialize point global position
  ...
  if (mono_allframe_Obj_depth_init && whether_dynamic_object) {
    ...
    if (triangulate_dynamic_pts) {
      ...
      for (size_t i = 0; i < frameMapPointMatches.size(); i++) {
        ...
        if (pMP && pMP->is_dynamic) // the point is matched to this frame, and
                                    // also dynamic.
        {
          // check if this point is created by last keyframe, if yes,
          // triangulate it with this frame!   if created earlier, not need
          if (!pMP->is_triangulated) // if not Triangulated
          {
            ...
            if (use_dynamic_klt_features)
              objectLastframe =
                  mpLastKeyFrame->local_cuboids
                      [mpLastKeyFrame->keypoint_associate_objectID_harris
                           [pixelindLastKf]];
            else
              objectLastframe =
                  mpLastKeyFrame->local_cuboids
                      [mpLastKeyFrame
                           ->keypoint_associate_objectID[pixelindLastKf]];
            ...
            // get new cube pose in this frame??? based on keypoint object
            // asscoiate id.
            ...
            g2o::cuboid cube_pose_now =
                objectThisframe
                    ->GetWorldPos(); // current obj pose, not BA optimimized
            // check truth tracklet id.
            if (use_truth_trackid)
              if (objectLastframe->truth_tracklet_id !=
                  objectThisframe->truth_tracklet_id) {
                ROS_ERROR_STREAM("Different object tracklet id, possibly due "
                                 "to wrong KLT point tracking");
                continue;
              }
            g2o::SE3Quat objecttransform =
                cube_pose_now.pose * cube_pose_lastkf.pose.inverse();
            cv::Mat Tcw_now_withdynamic =
                Tcw_now * Converter::toCvMat(objecttransform);
    ...
    if (1) // randomly select N points, don't initialize all of them
    {
      ...
      if (actually_use_obj_depth) {
        ...
        if (use_dynamic_klt_features) {
          ...
          // 		    objmask_img:  0 background, >0 object areas   change
          // to--->   255 object areas. 0 background.
          ...
          vector<pair<int, cv::Point2f>> exist_obj_mappts; // point, obverse
                                                           // time

          for (size_t i = 0; i < mCurrentFrame.mvpMapPointsHarris.size(); i++) {
            MapPoint *pMP =
                mCurrentFrame
                    .mvpMapPointsHarris[i]; // TODO later should be separate
                                            // mappoint for dynamic
            if (pMP && pMP->is_dynamic) {
              exist_obj_mappts.push_back(make_pair(
                  pMP->Observations(), mCurrentFrame.mvKeysHarris[i].pt));
            }
          }
          // sort(exist_obj_mappts.begin(), exist_obj_mappts.end(), [](MapPoint
          // *a, MapPoint *b) { return a->Observations() > b->Observations();
          // });
          sort(exist_obj_mappts.begin(), exist_obj_mappts.end(),
               [](const pair<int, cv::Point2f> &a,
                  const pair<int, cv::Point2f> &b) {
          ...
          cv::goodFeaturesToTrack(mpCurrentKeyFrame->raw_img, corners,
                                  max_new_pts, 0.1, MIN_DIST, good_mask);
          ...
          mpCurrentKeyFrame->mvpMapPointsHarris =
              mCurrentFrame.mvpMapPointsHarris;
          ...
          mpCurrentKeyFrame->keypoint_associate_objectID_harris.resize(
              totalfeat);
          ...
          for (int new_fea_ind = 0; new_fea_ind < numNewfeat; new_fea_ind++) {
            ...
            cv::Mat x3D = mpCurrentKeyFrame->UnprojectPixelDepth(
                corners[new_fea_ind], point_depth);
          ...
          std::vector<int>
              has_object_depth_pixel_inds; // points with no depth yet but with
                                           // matching object and plane
          ...
          for (size_t i = 0; i < vpMapPointMatches.size(); i++) {
            ...
            {
              int gridx, gridy;
              if (mpCurrentKeyFrame->PosInGrid(mpCurrentKeyFrame->mvKeys[i],
                                               gridx, gridy)) {
          ...
          cout << "all points to initilaze  "
               << has_object_depth_pixel_inds.size() << "  initialized "
               << max_initialize_pts << endl;
          int nPoints = 0;

          if (whether_actually_planeobj_init_pt) {
            srand(time(NULL));
            // 		    random_shuffle (
            // has_object_depth_pixel_inds.begin(),
            ...
            while ((nPoints < max_initialize_pts) &&
              ...
              if ((point_depth < 0)) {
                if (mpCurrentKeyFrame->keypoint_associate_objectID[pixel_ind] >
                    -1) {
                  point_depth =
                      mpCurrentKeyFrame
                          ->local_cuboids
                          ...
                          ->cube_meas.translation()[2]; // camera z
                  x3D =
                      mpCurrentKeyFrame->UnprojectDepth(pixel_ind, point_depth);
                }
              }
              if (point_depth > 0) {
                MapPoint *pNewMP = new MapPoint(x3D, mpCurrentKeyFrame, mpMap);
    ...
    // mCurrentFrame.mvpMapPoints indepent of new created mappoints in
    // localmapping
  }
  ...
  if (enable_ground_height_scale) // true
  {
    ...
    // do it in every frame, otherwise may take longer time when do it together
    // for many frames.
    for (size_t iMP = 0; iMP < mCurrentFrame.mvpMapPoints.size(); iMP++) {
      if (pKF->mvKeysUn[iMP].pt.x > img_width / ground_roi_middle &&
          pKF->mvKeysUn[iMP].pt.x <
              img_width / ground_roi_middle * (ground_roi_middle - 1))
    ...
    if (pKF->mnId % ground_everyKFs == 0) {
      ...
      for (size_t ii = 0; ii < mvpLocalKeyFrames.size(); ii++) {
        ...
        if (pKFi->mnId >= anchor_frame_kfid) {
          ...
          if (pKFi->mnId < minKFid) {
            minKFid = pKFi->mnId; // the original anchor frame id might not
                                  // exist due to culling.
            first_keyframe = pKFi;
          }
        }
      }
      ...
      KeyFrame *median_keyframe =
          nullptr; // scale relative to the center frames instead of the
                   // begining frame? more accurate?
      if (height_esti_history.size() > 0) {
        ...
        median_keyframe =
            first_keyframe; // still want to scale at the very first fram
      ...
      if ((!recently_have_object) || (height_esti_history.size() < 1)) {
        ...
        for (size_t i = 0; i < ground_local_KFs.size(); i++) {
          ...
          for (size_t j = 0;
               j < ground_local_KFs[i]->ground_region_potential_pts.size();
               j++) {
            if (ground_local_KFs[i]->ground_region_potential_pts[j] >=
                framepointmatches.size()) {
            }
            MapPoint *pMP =
                framepointmatches[ground_local_KFs[i]
                                      ->ground_region_potential_pts[j]];
            if (pMP)
              if (!pMP->isBad())
                if (pMP->mnGroundFittingForKF != pKF->mnId) {
                  cv::Mat point_position =
                      pMP->GetWorldPos(); // fit plane in global frame, then
                                          // tranform plane. saving time for
        ...
        std::cout << "Potential plane pt size    "
                  << potential_plane_points.size() << "   "
                  << ground_local_KFs.size() << std::endl;

        // TODO can we directly search height plane to find points supporting
        // it?? not using ransac. Song used it.
        ...
        if (height_esti_history.size() > 0)
          seg->setDistanceThreshold(ground_dist_ratio *
                                    height_esti_history.back());
        else
        ...
        Eigen::Vector4f global_fitted_plane(
            coefficients.values[0], coefficients.values[1],
            coefficients.values[2], coefficients.values[3]);
        float cam_plane_dist, angle_diff_normal;

        // transform to anchor frame
        KeyFrame *anchor_frame =
            first_keyframe; // first_keyframe  median_keyframe  pKF;
        cv::Mat anchor_Tcw = anchor_frame->GetPose();
        cv::Mat anchor_Twc = anchor_frame->GetPoseInverse();

        // take averge of all camera pose dist to plane,  not just wrt anchor
        // frame
        if (1) {
        ...
        ROS_WARN_STREAM("Find init plane  dist   "
                        << cam_plane_dist << "   angle  " << angle_diff_normal
          ...
          if (angle_diff_normal < 10) {
                ...
                (height_esti_history.size() < 4)) // or compare with last time?
            {
              ...
              for (size_t i = 0; i < inliers.indices.size(); i++)
                potential_plane_points[inliers.indices[i]]
                    ->ground_fitted_point = true;

              float final_filter_height = cam_plane_dist;
              // take average or recent tow/three frames. or median filter? is
              // this correct if object scale???
              if (height_esti_history.size() > 2) {
                final_filter_height = 0.6 * height_esti_history.back() +
                                      0.4 * filtered_ground_height;
              }
              ...
              if (enable_ground_height_scale) // true
              {
                for (size_t iMP = 0; iMP < mvpLocalMapPoints.size();
                     iMP++) // approximatedly. actually mvpLocalMapPoints has
                            // much more points
                {
                ...
                // loop over mlpReferences, if any frames' references frames lie
                // in this range, scale the relative poses accordingly
                // mlpReferences doesn't include the initialization stage... //
                // if it is bad...??
                for (size_t ind = first_keyframe->mnFrameId -
                                  initializer_starting_frame_id;
...
void Tracking::SearchLocalPoints() {
  ...
  if (nToMatch > 0) {
    ORBmatcher matcher(0.8); // TODO  if point is from my depth init, whether
    ...
    matcher.SearchByProjection(mCurrentFrame, mvpLocalMapPoints,
                               searchRadiusFactor);
  }
}
...
void Tracking::UpdateLocalMap() {
  ...
  mpMap->SetReferenceMapPoints(mvpLocalMapPoints); // darkgray
...
void Tracking::UpdateLocalKeyFrames() {
  ...
  //            KeyFrame *run_detect_pkf= localKeyFrames.front();
  //            std::set<MapPoint *> points = run_detect_pkf->GetMapPoints();
  //            cout<<"更新关键帧id: "<< run_detect_pkf->mnId<< endl;
  ...
  //                        size_t>::const_iterator it = observations.begin(),
  //                        itend = observations.end(); it != itend; it++)
  //                            keyframeCounter[it->first]++;
  //                    }
  ////                else
  ////                {
  ////                    (*run_detect_pkf)->mvpMapPoints[i] = NULL;
  ////                }
  //                }
  //            }
  //        }
  ...
  //        iter=localKeyFrames.begin();iter!=localKeyFrames.end();iter++){
  //            keyframeCounter[*iter]++;
  //        }
  //    }
  ...
  //    iter=keyframeCounter.begin();iter!=keyframeCounter.end();iter++){
  //        cout<<iter->first->mnId<<" ";
  //    }
  //    cout<<endl;
...
void Tracking::Reset() {
  ...
  mpLocalMapperLater->RequestReset();
  cout << " done" << endl;
