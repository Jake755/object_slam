...
int main(int argc, char **argv)
{
    ...
    {
        cerr << endl
             << "ERROR! Usage: rosrun ORB_SLAM2 Mono path_to_vocabulary path_to_settings" << endl;
        ros::shutdown();
        return 1;
    }
    ros::NodeHandle nh;
    ...
    std::string packagePath = ros::package::getPath("object_slam");
    ...
    ORB_SLAM2::System SLAM(argv[1], argv[2], argv[3], ORB_SLAM2::System::MONOCULAR, enable_loop_closing);

    ImageGrabber igb(&SLAM);

    // KITTI rosbag /camera/image_raw
    // TUM rosbag /camera/rgb/image_color
    ros::Subscriber sub = nh.subscribe("/camera/image_raw", 10, &ImageGrabber::GrabImage, &igb); // GarbImage then to Track
