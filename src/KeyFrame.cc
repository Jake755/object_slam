...
void KeyFrame::addObjObNums() {
  for (std::map<MapPoint *, int>::iterator iter = save4addObjObLater.begin();
       iter != save4addObjObLater.end(); iter++) {
    if (keypoint_associate_objectID.size() > 0) {
      int frame_cuboid_id =
          keypoint_associate_objectID[iter->second]; // 哪一个点对应的ID
      if (frame_cuboid_id > -1) {
        iter->first->AddObjectObservation(local_cuboids[frame_cuboid_id]);
      }
    }
  }
}
  ...
  for (size_t i = 0; i < mvpMapPoints.size(); i++)
    if (mvpMapPoints[i])
      mvpMapPoints[i]->EraseObservation(
          this); // don't need to frame.EraseMapPointMatch,because this frame is
                 // already set bad! won't use it
...
float KeyFrame::ComputeSceneMedianDepth(const int q) {
  ...
  std::partial_sort(vDepths.begin(), vDepths.begin() + check_index,
                    vDepths.end());
