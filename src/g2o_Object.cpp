...
SE3Quat exptwist_norollpitch(const Vector6d &update) {
  ...
  Matrix3d R;
  R << cos(omega(2)), -sin(omega(2)), 0, sin(omega(2)), cos(omega(2)), 0, 0, 0,
      1;
...
void VertexCuboid::oplusImpl(const double *update_) {
  ...
  if (whether_fixrotation) {
    newcube.pose.setTranslation(_estimate.pose.translation() +
                                update.segment<3>(3));
    ...
    newcube.pose =
        _estimate.pose *
        exptwist_norollpitch(
            update2.head<6>()); // NOTE object pose is from object to world!!!!
  } else
    newcube.pose = _estimate.pose * SE3Quat::exp(update.head<6>());

  if (whether_fixheight) // use previous height
    newcube.setTranslation(Vector3d(newcube.translation()(0),
                                    _estimate.translation()(1),
...
void EdgeSE3CuboidFixScaleProj::computeError() {
  const VertexSE3Expmap *SE3Vertex = dynamic_cast<const VertexSE3Expmap *>(
      _vertices[0]); //  world to camera pose
  const VertexCuboidFixScale *cuboidVertex =
      dynamic_cast<const VertexCuboidFixScale *>(
...
void EdgeDynamicPointCuboidCamera::computeError() {
  ...
  const VertexCuboidFixScale *cuboidVertex =
      dynamic_cast<const VertexCuboidFixScale *>(
          _vertices[1]); // object to world pose
  const VertexSBAPointXYZ *pointVertex =
      dynamic_cast<const VertexSBAPointXYZ *>(
          _vertices[2]); // point to object pose

  Vector3d localpt = SE3Vertex->estimate() *
                     (cuboidVertex->estimate().pose * pointVertex->estimate());

  Vector2d projected =
      Vector2d(Kalib(0, 2) + Kalib(0, 0) * localpt(0) / localpt(2),
...
void EdgeDynamicPointCuboidCamera::linearizeOplus() {
  ...
  Matrix<double, 2, 3>
      projptVscamerapt; // 2d projected pixel / 3D local camera pt
  ...
  // jacobian of point
  _jacobianOplus[2] =
      -projptVscamerapt * combinedT.rotation().toRotationMatrix();
...
void EdgeObjectMotion::computeError() {
  const VertexCuboidFixScale *cuboidVertexfrom =
      dynamic_cast<const VertexCuboidFixScale *>(
          _vertices[0]); // object to world pose
  const VertexCuboidFixScale *cuboidVertexto =
      dynamic_cast<const VertexCuboidFixScale *>(
          _vertices[1]); // object to world pose
  const VelocityPlanarVelocity *velocityVertex =
      dynamic_cast<const VelocityPlanarVelocity *>(
          _vertices[2]); // object to world pose

  if (cuboidVertexfrom == nullptr || cuboidVertexto == nullptr ||
      velocityVertex == nullptr)
  ...
  Vector2d velocity =
      velocityVertex->estimate(); // v w   linear velocity and steer angle
  ...
  Vector3d trans_back_pred =
      posefrom.translation() + (velocity(0) * delta_t - vehicle_length * 0.5) *
                                   Vector3d(cos(yaw_from), sin(yaw_from), 0);
  double yaw_pred =
      yaw_from + tan(velocity(1)) * delta_t / vehicle_length * velocity(0);

  Vector3d trans_pred =
      trans_back_pred +
      vehicle_length * 0.5 * Vector3d(cos(yaw_pred), sin(yaw_pred), 0);
...
Vector3d cuboid::point_boundary_error(const Vector3d &world_point,
  ...
  // transform the point to local object frame  TODO actually can compute
  // gradient analytically...
  Vector3d local_pt =
      point_scale * (this->pose.inverse() * world_point)
                        .cwiseAbs(); // change global point to local cuboid body
                                     // frame.  make it positive.
  Vector3d error;

  // if point is within the cube, error=0, otherwise penalty how far it is
  // outside cube
  for (int i = 0; i < 3; i++) {
    if (local_pt(i) < this->scale(i))
      ...
      error(i) =
          ...
          this->scale(
              i); // if points two far, give a constant error, don't optimize.
  }
...
void EdgePointCuboidOnlyObject::computeError() {
  ...
  for (size_t i = 0; i < object_points.size();
       i++) // use abs  otherwise   pos neg will counteract by different pts.
            // maybe each edge one pt?
    point_edge_error +=
        estimate_cube
            .point_boundary_error(object_points[i], max_outside_margin_ratio)
            .cwiseAbs();
  if (object_points.size() > 0)
    point_edge_error = point_edge_error / object_points.size();

  point_edge_error =
      point_edge_error.array() / estimate_cube.scale.array(); // scale it
  ...
  if (prior_object_half_size(0) >
      0) // if prior shape is being set, such as KITTI, then give large weight
         // for shape error
  {
    prior_weight = 50.0;
    prior_shape_error =
        ((estimate_cube.scale - prior_object_half_size).array() /
...
void EdgePointCuboid::computeError() {
  ...
  Vector3d point_edge_error =
      estimate_cube
          .point_boundary_error(pointVertex->estimate(),
                                max_outside_margin_ratio)
...
void UnaryLocalPoint::computeError() {
  ...
  // if point is within the cube, point_edge_error=0, otherwise penalty how far
  // it is outside cube
  for (int i = 0; i < 3; i++) {
