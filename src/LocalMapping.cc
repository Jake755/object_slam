...
LocalMapping::LocalMapping(Map *pMap, const float bMonocular)
    : mbMonocular(bMonocular), mbResetRequested(false),
      ...
      mbAcceptKeyFrames(true), printLog(false) {}
...
void LocalMapping::bundle_adjustment_caller() {
  ca::Profiler::tictoc("HBA");
  Optimizer::LocalBundleAdjustment(mpCurrentKeyFrame, &mbAbortBA, mpMap);
  ca::Profiler::tictoc("HBA");
}

void LocalMapping::bundle_adjustment_caller_obj(KeyFrame *pKF) {
  if (whether_detect_object && bundle_object_opti) {
    ca::Profiler::tictoc("BA Total Object");
    bool flag = false;
    if (whether_dynamic_object)
      Optimizer::LocalBACameraPointObjectsDynamic(pKF, &flag, mpMap, false,
                                                  false);
    else
      Optimizer::LocalBACameraPointObjects(pKF, &flag, mpMap, false, false);
    ca::Profiler::tictoc("BA Total Object");
  } else {
...
bool LocalMapping::RunMappingIteration() {
  ...
  if (CheckNewKeyFrames()) // CheckNewKeyFrames()
  {
    ...
    MapPointCulling();
    mpCurrentKeyFrame->mlpRecentAddedMapPoints = mlpRecentAddedMapPoints;
...
void LocalMapping::ProcessNewKeyFrame() {
  ...
  const vector<MapPoint *> vpMapPointMatches =
      mpCurrentKeyFrame->GetMapPointMatches(); // 点从CreateNewMapPoints()来
...
void LocalMapping::MapPointCulling() {
  ...
  if (printLog) {
    std::cout << "mlpRecentAddedMapPoints culling: "
...
void LocalMapping::KeyFrameCulling() {
  ...
  for (vector<KeyFrame *>::iterator vit = vpLocalKeyFrames.begin(),
    ...
    KeyFrame *pKF = *vit;
    if (pKF->mnId == 0 ||
        !pKF->IsCreatedObjs) // TODO DONE. 若关键帧物体尚未构建则不去除
      continue;
