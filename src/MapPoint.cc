...
cv::Mat MapPoint::GetWorldPos() {
  unique_lock<mutex> lock(mMutexPos);
  if (is_dynamic &&
      best_object != nullptr) // for dynamic pt, transfrom from object pose.
  {
    return Converter::toCvMat(
        best_object->GetWorldPos().pose.map(Converter::toVector3d(PosToObj)));
  }
...
cv::Mat MapPoint::GetWorldPosBA() {
  ...
  if (is_dynamic && best_object != nullptr) {
    return Converter::toCvMat(
        best_object->GetWorldPosBA().pose.map(Converter::toVector3d(PosToObj)));
  }

  return cv::Mat::zeros(
      0, 0, CV_32F); // should not happen, this function only for dynamic point
}

Eigen::Vector3f MapPoint::GetWorldPosVec() {
  cv::Mat worldpose = GetWorldPos();
  return Eigen::Vector3f(worldpose.at<float>(0), worldpose.at<float>(1),
                         worldpose.at<float>(2));
}
...
void MapPoint::AddObservation(KeyFrame *pKF, size_t idx) {
  ...
  if (!use_dynamic_klt_features) {
    if (pKF->mvuRight[idx] >= 0)
      nObs += 2; // a stereo/RGBD camera projects to left/right camera. in g2o
                 // BA, both left/right is used. useful for constraint scale.
    else
      nObs++;
  } else
    nObs++;

  // NOTE points usually initially associated different objects, due to
  // occlusions
  if (associate_point_with_object) {
    pKF->save4addObjObLater.insert(pair<MapPoint *, int>(this, idx));
    //        if (use_dynamic_klt_features && is_dynamic)
    //        {
    //            if (pKF->keypoint_associate_objectID_harris.size() > 0)
    //            {
    //                int frame_cubod_id =
    //                pKF->keypoint_associate_objectID_harris[idx]; if
    //                (frame_cubod_id > -1)
    ...
    //            {
    //                int frame_cubod_id =
    //                pKF->keypoint_associate_objectID[idx]; if (frame_cubod_id
    //                > -1)
    //                    AddObjectObservation(pKF->local_cuboids[frame_cubod_id]);
    //            }
    //        }
...
void MapPoint::EraseObservation(KeyFrame *pKF) {
  bool bBad = false;
  {
    ...
    if (mObservations.count(pKF)) {
      ...
      if (nObs <= 2)
        bBad = true;
      // TODO DONE. remove object observation...
      if (associate_point_with_object) {
        pKF->save4addObjObLater.erase(this);
      }
    }
  }
...
void MapPoint::AddObjectObservation(MapObject *obj) {
  ...
  if (obj->already_associated) {
    ...
    if (MapObjObservations[objlandmark] > max_object_vote) {
      ...
      best_object->AddUniqueMapPoint(
          this, max_object_vote); // even being added before, still increase
                                  // observation num
    }
  } else {
    LocalObjObservations.insert(obj);
    obj->AddPotentialMapPoint(
        this); // for frame local object, still add point observation. will be
               // useful when local object changes to landmark.
  }
}
...
void MapPoint::FindBestObject() {
  // for loop to update best beloned object, only for object landmarks, frame
  // local object is not considered.
  best_object = nullptr;
  max_object_vote = 0;
  for (map<MapObject *, int>::iterator mit = MapObjObservations.begin(),
                                       mend = MapObjObservations.end();
       mit != mend; mit++)
  ...
  for (map<KeyFrame *, size_t>::iterator mit = obs.begin(), mend = obs.end();
    ...
    if (use_dynamic_klt_features && is_dynamic)
      ...
      pKF->EraseMapPointMatch(
          mit->second); // remove observations from frame side
  }
...
void MapPoint::Replace(MapPoint *pMP) {
  ...
  map<KeyFrame *, size_t> obs;
  {
    ...
    // lin755
    pMP->MapObjObservations.insert(MapObjObservations.begin(),
                                   MapObjObservations.end());
  }
  ...
  for (map<KeyFrame *, size_t>::iterator mit = obs.begin(), mend = obs.end();
    ...
    if (!pMP->IsInKeyFrame(pKF)) {
      //            pKF->ReplaceMapPointMatch(mit->second, pMP); // lin755
      pMP->AddObservation(pKF, mit->second);
    } else {
      //            pKF->EraseMapPointMatch(mit->second); // lin755
    }
  }
  //    pMP->IncreaseFound(nfound); // lin755
  //    pMP->IncreaseVisible(nvisible); // lin755
  //    pMP->ComputeDistinctiveDescriptors(); // lin755
  //
  //    mpMap->EraseMapPoint(this); // lin755
}
