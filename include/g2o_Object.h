...
public:
  ...
  cuboid rotate_cuboid(
      double yaw_angle) const // to deal with different front surface of cuboids
  {
    cuboid res;
    SE3Quat rot(
  ...
  cuboid timesInverse(const cuboid &othercube) const {
    ...
    Matrix4d result_homomat =
        current_homomat * other_homomat.inverse(); // [RS, t]
    Matrix3d result_rot =
  ...
  Matrix2Xd projectOntoImage(const SE3Quat &campose_cw,
  ...
  Vector3d point_boundary_error(const Vector3d &point,
...
class VertexCuboid
    : public BaseVertex<9,
                        cuboid> // NOTE  this vertex stores object pose to world
{
public:
...
class VertexCuboidFixScale
    : public BaseVertex<6, cuboid> // less variables. should be faster  fixed
                                   // scale should be set
{
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
...
// camera -fixscale_object 2D projection error, rectangle, could also change to
// iou
class EdgeSE3CuboidFixScaleProj
    : public BaseBinaryEdge<4, Vector4d, VertexSE3Expmap,
                            VertexCuboidFixScale> {
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
...
public:
  ...
  double get_error_norm() // for debug
  {
    ...
    const VertexCuboid *cuboidVertex =
        static_cast<const VertexCuboid *>(_vertices[1]);
...
public:
  ...
  void computeError() {
    const VertexCuboid *cuboidVertex = static_cast<const VertexCuboid *>(
        _vertices[0]); //  object pose to world
  ...
  double get_error_norm() {
    const VertexCuboid *cuboidVertex =
        static_cast<const VertexCuboid *>(_vertices[0]);
...
class EdgePointCuboid
    : public BaseBinaryEdge<3, Vector3d, VertexSBAPointXYZ, VertexCuboid> {
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
  EdgePointCuboid(){};
  ...
  double max_outside_margin_ratio; // truncate the error if point is too far
                                   // from object
};

// object point surface error, both will optimize.   object has fixed size.
class EdgePointCuboidFixScale
    : public BaseBinaryEdge<3, Vector3d, VertexSBAPointXYZ,
                            VertexCuboidFixScale> {
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
...
// a local point in object frame. want it to lie inside cuboid. only point
// object dimension is fixed. basically project the point onto surface
...
// dynamic point attached to cuboid, then project to camera to minimize
// reprojection error.
...
// one object connected with all fixed points. only optimize object.  want
// object to contain points
class EdgePointCuboidOnlyObject
    : public BaseUnaryEdge<3, Vector3d, VertexCuboid> {
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
  ...
  double max_outside_margin_ratio;     // truncate the error if point is too far
                                       // from object
  Vector3d prior_object_half_size; // give a prior object size, otherwise a huge
                                   // cuboid can always contain all points
};

// one object connected with all fixed points   similar as above
class EdgePointCuboidOnlyObjectFixScale
    : public BaseUnaryEdge<3, Vector3d, VertexCuboidFixScale> {
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
