...
public:
    ...
    Tracking(System *pSys, ORBVocabulary *pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Map *pMap,
             KeyFrameDatabase *pKFDB, const std::string &strSettingPath, const string &strYoloModelPath, const int sensor);
    ...
    void SetLocalMapperLater(LocalMappingLater *pLocalMapperLater);
...
public:
    ...
    Frame mCurrentFrame;
    cv::Mat mImGray;
    cv::Mat mImColor; // for yolact

    //localKeyFrames
    std::mutex mMutexLocalKFs;
    std::list<KeyFrame *> localKeyFrames;
    void localKFListRemove(KeyFrame *pKF);
...
protected:
    ...
    void checkValidLocalObjKFs(); // lin755
    ...
    LocalMappingLater *mpLocalMapperLater;
    ...
    std::vector<KeyFrame *> mvpLocalKeyFrames; // local visibile or co-visible keyframes../ not include just created KF
    std::vector<MapPoint *> mvpLocalMapPoints; // for track with local map
    std::vector<KeyFrame *> LocalObjKeyFrames; // lin755 局部物体关键帧
