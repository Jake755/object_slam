...
public:
  ...
  // corresponding to the idx keypoint of keyframe pKF, not the absoluate
  // coordinate
  void AddObservation(KeyFrame *pKF, size_t idx);
  void EraseObservation(KeyFrame *pKF); // called in Keyframe when set bad, and
                                        // in optimizer when removing outliers
  ...
  void
  Replace(MapPoint *pMP); // usually means replace close point, with similar
                          // decriptor. called in matcher.Fuse(), in localmapping
  ...
  void
  ComputeDistinctiveDescriptors(); // select/update map point's decriptors based
                                   // on frame's keypoints' decriptors
  ...
  int GetBelongedObject(
      MapObject *&obj);           // change obj, return observation times.
  MapObject *GetBelongedObject(); // return obj

  void AddObjObLater(); // lin755
...
public:
  ...
  cv::Mat PosToObj; // 3d point relative to object center. ideally will
                    // converge/fix after BA

  bool is_triangulated =
      false; // whether this point is triangulated or depth inited?
  bool is_optimized = false;

  MapObject *best_object; // one point can only belong to at most one object
  int max_object_vote;    // sometimes point is wrongly associated to an object.
                          // need more frame observation
  std::set<MapObject *>
      LocalObjObservations; // observed by local objects which hasn't become
